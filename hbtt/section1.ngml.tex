...
values of type \caltypeT{}.  There may also be a type 
...
\spsubsection{Gatekeeper Code and Fragile Code}
...
to be a corresponding segment of gatekeeper code which
...
We then have three options: test that systolic-greater-than 
...
to emerge as individual functions and datatypes are
...
the \q{discovery of questions} effect.  Systems in operation will be
...
separate and apart from that procedures own implementation.
...
\begin{displayquote}Ivory's type system is shallowly embedded within Haskell's
...
\cite[p. 1]{ivory}.
\end{displayquote}   In other words, the creators of Ivory are promoting the
...
Sections \sectsym{}\hyperref[sectTwo]{2}
and \sectsym{}\hyperref[sectThree]{3}.
}
