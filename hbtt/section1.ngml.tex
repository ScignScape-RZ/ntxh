...
species (Todor Ganchev, this volume). Here again feature 
...
integration.  In the case of testing, many of the scenarios 
...
we can imagine future homes/apartments provisioned with a panoply 
...
connected to their in-house product suite, but technology inside 
...
and proper transformations.  In lieu of \q{data-centric} 
...
\p{Teixera's chapter (this volume) considers voice-activated 
...
translation) with Dependency Grammar parses 
(see page).  Textual content can also be annotated with 
...
are often notated via some version of the \CoNLL{} 
(Conference on Computational Natural Language Learning) 
format, which textually serializes parse and 
...
Part of Speech and syntactic connections with other 
...
language patterns.  The \NLP{} components can be 
...
\NLP{} capabilities directly; instead, \NLP{} would be 
a distinct program receiving audio input and generating 
text and parse transcriptions which would be sent 
...
audio input (perhaps allowing users to pay back 
instructions they utterred in the past).  A relevant 
...
This extra data might be annotations such as how 
...
data types, such as (1) a class for \UDPipe{} sentences 
obtained from \CoNLLU{} (2) a class for audio samples; 
(3) speaker and time/location information; plus 
versions of these classes appropriate for 
\GUI{}s and database persistence.
}
\p{Likewise, we 
can anticipate the procedures which these 
...
are meaningful in the context of the length of 
