...
channel \q{bundle} \mdash{} which, in the current example, has
...
between code graphs, a channel bundle can be seen as a description
of how two distinct code-graphs may be connected via carrier
transfers.  When a function is called, there is a channel
complex which I'll call a \i{package} that supplies values to
the channel \i{bundle}.  In the concrete example, the
...
\i{bundle}: a collection of transfers \carrOneOpTransferTwo{}
together describe an overall transfer between a
\i{package} and a \i{bundle}.
}
...
a \i{Channel Bundle}, whereas complexes representing a function
\i{call} I am calling a \i{Channel Package}.  Input channels
are then those whose carrier transfers occur in the package-to-bundle
direction, whereas output channels are the converse.
}
\p{In addition to the package/bundle distinction, we can also
...
where they describe the general pattern of package/bundle
...
package or bundle, or a \i{runtime} package or bundle, respectively.
...
\p{This formal channel/complex/bundle/package/carrier vocabulary codifies
...
\p{At the same time, source code
