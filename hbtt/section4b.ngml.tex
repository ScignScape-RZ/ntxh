...
Assume that \fgroundch{} can be assigned a flag which suppresses
...
on the runtime side.  The hook into this \Cpp{} runtime code 
...
point in this code is the instruction at {\TwoOverlay}, 
...
At {\ThreeOverlay}, a block itself is assigned to a 
\q{callable value} type (cf. the \q{cv} initials).  The 
arguments identified for the kernel procedure then ({\OneOverlay}), 
then, alternate between encapsulations of by-need expressions 
...
In that spirit, the demo code-set focuses
