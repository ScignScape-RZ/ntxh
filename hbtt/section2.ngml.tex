...
\p{Thus far, I have written in general terms about 
architectural features related to CyberPhysical software; 
in particular, verifying coding assumptions concerning 
...
is distributed among many individual procedures, which 
are interconnected.  Each procedure, in general, 
...
when some condition holds, but call diffdrent procedures 
...
\p{There are perhaps three different perspectives which can be 
taken toward such a comprehensive theory.  One route is 
...
consider procedures as instances of funtional types, 
characterized by sets of inputs and output types.  
A procedure is then a transform which, in the presence 
of (zero or more) inputs having the proper types, produces 
(one or more) outputs with their own types.  In practice, 
some procedures do not return values, but they \i{do} have 
some kind of side-effect, which can be analyzed as a 
variety of \q{output}.  Finally, procedures can be studied 
via mathematical frameworks such as the Lambda Calculus, 
which allows notions of functions on typed parameters, and 
of functional application \mdash{} apply functions to concrete 
values, which is analogous to calling procedures with 
concrete input arguments \mdash{} to be made formally rigorous.
}
\p{I will briefly consider all three of perspectives \mdash{} Source Code 
...
\spsubsection{Generalized Lambda Calculus}
...
Consider the visual differences between mathematical 
...
of course the bread-and-butter of mathematical discovery.  In terms 
...
value, like $\pi{}$ \mdash{} and that free symbols (like \rRad{}) can be 
bound to other formulae, yielding new equations. 
}
\p{From cases like these \mdash{} relative simple geometric 
expressions \mdash{} mathematicians began to ask foundation 
questions about mathematical formulae: what are all formulae 
that can be built up from a set of core equations via 
repetedly substituting nested expressions for 
free symbols?  This questions turns out to be related to 
...
to plug in to those formulae, the set of unkowns decreases.  
...
a computation which loops indefinitely.\footnote{Although there are sometimes techniques for converting formulae 
...
the John von Neumann formal prototypes for digital 
computer.  
}
...
for computating machines.  Mathematicians in the decades before WWII
...
`p`
It was only after the emergence of modern digital computers 
...
abstractly interchangable, but it is important to 
reprecent the contrast between, say, 
...
may new constructs which determine differet 
...
and document coding assumptions.  
}
...
definitions (recall the idea that \CH{} hypernodes are roughly analogous to
...
modeled by suitably typed \DH{}s as well.  So, for any given
procedure, assume that there is a corresponding \DH{} representation
...
Lambda Calculii with Source Code Ontologies.  
The essential construct in Lambda Calculii is 
...
graph operation, liking two nodes \mdash{} is then 
...
features such as Object-Orientation, exceptions, and so forth.  
...
Objects and exceptions, are certainly semantic 
...
\spsubsectiontwoline{Procedural Input/Output Protocols via Type Theory}
...
alphabets).\footnote{In other contexts, however, non-mathematical core types may be appropriate: for example,
...
channels are associated with different procedures may 
...
\item{} In all common languages which have exceptions, 
...
to forbid \exception{} channels from sharing values with 
most other channels (tied to any other procedure).  Instead, 
...
receive the enclosing method's Sigma channel unless 
a different object for the called method is supplied expressly. 
\item{} In the object-oriented technique known as \q{method chaining}, 
one procedures' \return{} channel is transferred to a 
subsequent procedures' \sCh{} channel.  The pairing of 
Return and Sigma channels therefore gives rise to one 
...
inter-function operators can be used in lieu of function-like values 
...
One particular channel formation therefore models 
the basic \mOldLambda{}-Calculus and, simultaneously, 
the minimal theory of function-like types 
(for Cartesian Closed type Categories).  
More complex channel combinations and protocols 
can then model more complex (or at least more 
modern) variations on \mOldLambda{}-Calculii and 
programming language type systems. 
}
\subsubsection{Channelized Type Systems}
\p{Collectively, to summarize my disuccion to this point, 
...
\lambda{} plus \return{} combination (with \return{} channels 
...
the use of code-graphs \mdash{} hypergraphs annotated according 
