...
when some condition holds, but call alternative procedures
...
\spsubsectiontwolinerepl{Directed Hypergraphs and \q{Channel Abstractions}}%
...
conceivably there are multiple 46-year-olds named Nathaniel,
so \i{that} blank node plays a key semantic role
(analogous to the quantifier in \q{\i{There is} a Nathaniel,
age 46...}); it provides an unambiguous nexus so that
further predicates can be attached to \i{one specific}
46-year-old Nathaniel rather than any old \NathFF{}.  But there is no
...
ethereal; the number 46 in one graph is indistinguishable from 46 in
...
\q{the number 46} from being subjects as well as objects.
...
The essential construct in Lambda Calculi is
...
modern) variations on \mOldLambda{}-Calculi and
