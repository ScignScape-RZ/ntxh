...
\p{My discussion toward the end of the last section focused on 
...
\p{Here I propose the language of \q{cognitive transforms} 
...
\p{Models based on Functional Type Theory are useful in this context 
...
\mdash{} e.g., nouns and propositions \mdash{} generates a collection 
...
\p{In practice, many functional parts of speech 
...
Given, say, 
\begin{sentenceList}\sentenceItem{} \label{itm:ambience} The city's ambience is colonial and the climate is tropical.
\udref{en_gum-ud-train}{GUM_voyage_merida-20}
\end{sentenceList}
the gloss \i{(and (is ((The ('s city)) ambience) colonial) (is (the climate) tropical))}
...
\begin{sentenceList}\sentenceItem{} \label{actually} The most popular lodging is actually camping on the beaches.
\udref{en_gum-ud-train}{GUM_voyage_socotra-16}
\end{sentenceList}
with gloss \gl{((actually is) ((The (most popular)) lodging) ((on (the beaches)) camping))}.  
...
of camping (qua verb) \mdash{} then we could treat the locative 
...
with conventional Dependency labels \mdash{} one option is to 
...
two annotation styles can be juxtaposed (figures \ref{fig:ambience} 
...
to a proposition, which could be notatated with something like \NNtoProp{}.  
...
explain \i{ungrammatical} constructions in \i{failure} for 
...
wisdom, such as sentences' requiring a noun and a verb (\SeqNPplVP{}).
...
autonomous: 
\begin{sentenceList}\sentenceItem{} \label{students} Many students and their parents 
came to complain about the tuition hikes.
\sentenceItem{} \label{office} Many students came by my office to complain about their grades.
\sentenceItem{} \label{after} Student after student complained about the tuition hikes.
\sentenceItem{} \label{parents} Student after student came with their parents to complain 
about the tuition hikes.
\end{sentenceList}
...
\p{As these sorts of examples illustrate, 
phrases cannot just be studied as full-scale replacements for semantic units.  Incorporating
...
forms of speech acts, like questions and commands. \input{figure.tex} In Figure ~\ref{fig:Iknow},
...
expression, canonically the whole sentence, is a 
