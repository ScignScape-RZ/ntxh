...
Given, say, 
\begin{sentenceList}\sentenceItem{} \swl{itm:ambience}{The city's ambience is colonial and the climate is tropical.}{sem}
...
\begin{sentenceList}\sentenceItem{} \swl{itm:actually}{The most popular lodging is actually camping on the beaches.}{sem}
...
to a proposition, which could be notatated with something like \NNtoProp{}.\footnote{A note on notation: I adopt the Haskell convention (referring to the Haskell
...
\PropToNYieldsN{}.\footnote{Numeric indices take the place of left and right adjecency 
...
autonomous: 
\begin{sentenceList}\sentenceItem{} \swl{students}{Many students and their parents 
...
\begin{sentenceList}\sentenceItem{} \swl{}{Three times, students asked an interesting question.}{sco}
...
pivotal in term of meaning, but they are the core skelaton of 
...
\begin{sentenceList}\sentenceItem{} \swl{}{The newspaper you are reading is being sued.}{lex}
...
be accepted:
\begin{sentenceList}\sentenceItem{} \swl{}{The newspaper fired a reporter and fell off
the table.}[(?)]{lex}
...
in those terms:
\begin{sentenceList}\sentenceItem{} \swl{}{Liverpool wants to sign a left-footed striker.}{ont}
...
}  At the same time, Petitot also links with a 
...
a large volumes Petitot co-edited.  \q{Cognitive} in this contexts 
tends to imply desire to groud analyses on holistic 
human experience, in its experiential, embodied, pragmatically-oriented, 
first-personal, and intersubjective dimensions.  In this sense 
...
philosophical commitment to the irredicubility of 
...
is \i{some} computable core of cognition, which scientists 
...
common perceptual pattern (and how they are 
...
rather than just descibing cognition as an experienced 
givenness \mdash{} plus both faithfulness to each person's 
conscious experience and also generality to many 
people's experience.  For example, Husserl's analyses of 
protention and retaining in sensing spatial form during 
perceptual episodes focused on discrete, extended objects 
(e.g. in \i{Thing and Space}), or his analyses of 
...
primordial consious content unfolds into the 
schematic and rational models of our surrouding environment 
...
\begin{sentenceList}\sentenceItem{} \swl{itm:footprints}{There are footprints on the beach.}{ref}
...
Meanwhile the footprints are presumably disjount, unlike train tracks.  
...
in different ways.  So we have, at a basic level, a 
contrast between situationally grounded or conceptually 
generalized references: 
\begin{sentenceList}\sentenceItem{} \swl{itm:Salesmen}{Salesmen are intelligent.}{ont}
...
chapter 6) are manifested in several 
...
between background and foreground; mixture of 
singular integrity and divisibility characterizing the 
foreground \mdash{} are taken for granted as patterns 
...
\begin{sentenceList}\sentenceItem{} \swl{itm:ink}{There are ink stains on the pages of this book.}{ref}
...
\p{The matrix of continutity/discontinuity, and individual coherence balanced 
against internal structuration and diversity, are \i{conceptually} 
intrinsic to notions like \i{ink stains} (or \i{footprints}), 
...
been \mdash{} something perceptually and enactively/kinaesthetically 
individuated \mdash{} or also a specific preperation of beer, 
...
link to discrete, perceptually/opertionally integral 
objects like glasses (though of course a glass of 
beer is also an integral complex where the beer and 
the glass are semi-autnomous parts).  The \q{two kinds} 
...
the latter one for Sauturnes, but I can imagine plausible 
conversations where these conventions would be reversed.  
}
\p{Type theory is a potential window onto how the 
...
pattern, as in: 
\begin{sentenceList}\sentenceItem{} \swl{itm:hall}{I am in the Hall of Fame.}{ref}
\sentenceItem{} \swl{itm:ontv}{I am on TV.}{ref}
\end{sentenceList}  
Nunberg argues that reference \q{transfers} between conceptually 
liked designata, so I refer to my car's location as my location, 
...
case like a painter \mdash{} referring to the location of her 
work in transit \mdash{} saying something like (his example 12):
\begin{sentenceList}\sentenceItem{} \swl{itm:crate}{I'm in the second crate on the right.}{ref}
\end{sentenceList}  
...
behind how and when such proxying works in a first-person context.
}
\p{On the other hand, I would argue that the pattern in 
(\ref{itm:hall})-(\ref{itm:ontv}) is not reducible to 
referential proxying as a logical maxim.  We could argue that 
...
\p{The relevant backstories govern the interpretations for first-person 
...
assumed competent with the relevant lexicon, minimally understands.  
...
object manifecting the speakers being elected) entering 
...
rise through the art world (or whatever autobiographical 
context preceded public appreciation) into the conversation.
} 
...
encounter her work; an a personality \i{on TV} means that the 
...
of an art work in a museum to the backstory of it provenance 
and the implication for the artist's career or place in history; 
...
we identify the canonical perceptually-oriented reference which 
supply a conceptual ground (the painting as perceptual 
object, the one-screen image as perceptual simulacrum); but then 
...
in interpretation.  In the proper context, I have no objection to 
\i{I am in that crate} (which Nunberg rejects).  
The gist there, as in (\ref{itm:Whitney}), is to establish conceptual 
focus onto a painting or art works, and to do so via a 
...
\p{In \i{I am in that crate} the situation is different: presumably 
...
Whiteney \mdash{} there is some structural resonance in how 
...
meaning and reference.  Failure to identify these 
