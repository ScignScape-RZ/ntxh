...
believe we should conceptualize hub sofware as 
...
than other kinds of CyberPhysical access 
...
items inside.  Consider then how we would design a 
User Interface networking with a \q{smart} regrigerator.  
...
(lines, say, showing a picture, price, text description, 
...
For example, with no room on-screen to show a 
complete list of food items \mdash{} and no obvious user 
...
snippets of information, in lieu of comprehensive access into a 
...
becomes instead a make-or-break 
User Experience necessity.  
}
...
take advantage of larger screens, with secondary 
application windows and other interactive features that 
we associate with native \GUI{} components.  It is a 
resonable hypothesis that this would render \AI{} less important: 
...
In that spirit, investments in \AI{} solutions could be 
...
\p{Hub applications could therefore be examples of what 
...
or coding errors which render the whole system suspect).  
...
governments are aggressively involved in scrutinizing 
medical practices; commercial activities (like marketing) are 
...
compilers and runtimes).  Unfortunately, 
...
I will examine real-world CyberPhysical data with a little more specificity.  
My goal in the following pages is to describe the sorts of 
details that need to be expressed in CyberPhysical data models 
and therefore verified in CyberPhysical code.  Whereas my subsequent 
analyses of code representation will adress \i{how} code can demonstrate 
