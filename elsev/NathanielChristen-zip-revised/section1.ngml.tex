...
data (a stadardized Electronic Medical Record format) must 
...
Health IT Certification, which maintains a network of software-evalution 
...
clinical innovations are to rethink 
analytic methodologies; or clinical technology 
is introduced which generates new kind of 
data \mdash{} reflecting the novel physical design 
of diagnostic equipment, for example \mdash{} 
...
(\CLG{}) model, developed at Montefiore Medical Center in New York 
\cite{EranBellin}.  
...
\q{cohort model} where patients are aggregrated 
...
\CLG{} object model would engeder its own \q{consume and query} 
protocols \mdash{} for data sharing, validation, and analysis \mdash{} 
which would be sufficiently divergent from \HLSeven{} formats 
that the \HLSeven{} certification process would not be 
applicable.  Comparable rigor would therefore 
...
given the (thankfully) fertile ground of research for new treatment and 
technologies, we can see our health care system as 
...
data models \mdash{} will constantly become outdated.  
For this reason Requirements Engineering should 
look more toward a \q{software-centric} paradigm, 
where developers consiously adopt design patterns 
