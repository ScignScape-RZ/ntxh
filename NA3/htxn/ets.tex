...
\usepackage[paperheight=17in,paperwidth=9.1in]{geometry}
...
\vspace{-1.5em}\input{etsf3.tex}
	\end{wrapfigure}
The previous case-study involving Thionyl Chloride exemplified 
a simple data structure transmitted between ETS plugins: 
specifically, the name of a single file to open.  
In other cases, the information sent between plugins would 
be more complex.  As a result, all ETS plugins need a 
common vocabulary for representing multi-part data 
structures, which is engineered by \EPF{}.  
...
they could conveniently refer back to those files 
as desired.  Suppose a student reading through the GRE Chemistry 
...
(which is question number \textit{95} in the test --- see 
...
SOCl$_2$ file --- see the Model View panel at 
the top-left on 
Figure~\bref{fig:i2}, where the Thionyl Chloride 
file is listed above the checked Lactose file 
(the latter is checked because it corresponds 
to the file currently visible in the view-port).}
%\input{etsf4.tex}
...
{\sectsp}
\mnoteb{\iEPF{} data\\in em-\\bedded files}\vspace{-.5em}
