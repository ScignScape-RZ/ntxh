...
{\makebox{The Latex/XML/Concurrent/RDF Infoset and Plugin Framework} \\\vspace*{10pt}(\lsLXCR{})}}}}
\end{center}
\end{minipage}
\end{tcolorbox}
\end{center}
...
{\sectsp}
\p{The following pages will summarize \LXCR{} (a Document 
...
Model defines a protocol for compiling and examining 
document structure, one which recognizes the distinct structural features 
of different manuscript formats, particularly 
...
including \RDF{} and Concurrent Markup which, 
technically, are structural extensions of \XML{}.  
...
represented by \XML{}'s Document Object Model.}

\p{In \XML{}, the \DOM{} is considered a protocol for 
accessing an \XML{} \textit{infoset}, which is an 
internal representation of \XML{} documents that 
abstracts from surface-level syntax.  Analogously, 
\LXCR{} documents have an internal structure which 
can be built from different kinds of source 
files; as such, different markup language may 
be used to create the same \LXCR{} document. 
In this manner, publishers or educational institutions can 
create their own \LXCR{}-compatible markup language --- either a 
specialized version of familiar languages 
such as \XML{}, or something entirely different. 
By compiling the markup to \LXCR{}, manuscripts 
in diverse surface-level formats can be 
used interoperably, or combined into larger documents.  
Because the source markup is abstracted from the 
internal representation, \LXCR{} data structures 
may be more precisely described as \q{infosets} 
rather than as \q{documents}.  Nevertheless 
(to sustain the analogy with \XML{}), 
the protocol for defining and accessing the 
structure of \LXCR{} infosets is called the \LXCR{} 
Document Object Model (\LDOM{}).
}

\p{Complementing this Document Object Model, 
the \LXCR{} Plugin Framework (\LPF{}) 
...
enhance the reading experience by giving readers 
convenient access to interactive, multimedia content.}

\mnote{\LXCR{} for multi-media and datasets}
\p{\lLXCR{} is designed for a 
...
software (both tools for authors to create documents and tools for readers to fully experience them) 
...
is especially relevant at a specific location in the 
publication text.  In the case of interactive \ThreeD{} views of a 
chemical compound (or analogously biological or physical  
entities such as viruses, tissue samples, vector fields, etc.), 
these assets are most helpful to the reader in conjunction with 
...
embedded at the point in a publication where the modeled protein is 
mentioned or analyzed.  It is not 
sufficient simply to note that a given 
multimedia file is a supplement to a given publication; 
instead, the link between the two resources 
has to be grounded at a fixed location in the 
text --- which is to say, at a specific point in the 
document structure.  With this information attached 
to the document structure, a 
publication viewer can add the visual cues and 
user actions to access the supplemental content 
--- for example, hyperrefs, buttons, or context menu 
options --- at document locations which are 
...
may be used in conjunction with software for composing and '
viewing publications.  In an educational context, 
\LPF{} plugins can augment scientific and 
technical applications with features that enhance 
their usefulness as teaching platforms.  
In particular, \LPF{} plugins can integrate desktop applications 
with course curricula, educational materials, 
and cloud services hosting student, instructor, 
and course information.  Overall, \LPF{} 
refers not to a single plugin but rather a 
...
\p{This proposed ETS Plugin Framework would create 
	a common code base that can be used to 
	implement ETS-specific plugins for 
	...
	ETS plugins would then help scientific applications 
	...
	concrete example, consider molecular 
	visualization software such as IQmol.  Via 
	an ETS plugin, IQmol could (1) render a \ThreeD{} 
	image when it receives data, in a chemical file format, 
	from e-readers with similar plugins; 
...
\p{Supplementing the functionality described in the 
	...
	would be a natural fit for Linguistic Technology 
	System's \q{Native Cloud/Native} 
	...
	The ETS plugin's back-end would also be 
...
\p{In addition, one feature which should be common to all ETS 
	...
	\q{My Account}.  The Account tab would review basic 
	information such as login credentials and schools 
	or programs where the student is registered. 
	Under the Courses tab, 
	students could select one course to focus on, 
...
To explain the features of the plugin suite, the following 
section will consider different scenarios where 
ETS plugins could contribute new technology 
benefiting student, instructors, authors, and publishers. 

\vspace{2em}
\noindent\lun{Use-Cases for the Proposed ETS Plugin Framework}
{\sectsp}

\p{To illustrate the functionality that would 
be implemented via ETS plugins, consider 
...
models of chemical compounds which can be viewed 
...
hosts an ETS plugin, the viewer would identify locations in the 
...
could identify the correct application to use for 
multimedia content based on its file type, perhaps 
in conjunction with data obtained from an ETS cloud service.  
Cloud-hosted data might, for instance, indicate 
that a specific course is using IQmol as a pedagogical 
tool, and request that cheminformatic files linked 
...
launch the correct application on the student's computer and 
...
or instructors' preferences (see the next scenario 
...
to how algorithms may traverse an \XML{} Infoset.  For 
...
not usually made available to the public (even to purchasers or 
...
can have several dimensions, including: 
...
such as practice tasks and assignments; 
...
Environment (\IDE{}) which is employed as a teaching tool]  
Using \IDE{}s is a prerequisite for most courses 
in computer science and computer programming, and 
\LPF{} plugins may be introduced in this context 
...
identified for Scenario 3: instructors may wish to foreground 
...
and the earlier use-cases is that Scenario 3 assumed 
...
\LXCR{}.  \lHTXN{} explicitly supports information 
