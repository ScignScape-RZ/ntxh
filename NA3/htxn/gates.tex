...
{Proposing a CORD-19 Software Development Kit Improve Machine Readability}
}}}
\end{center}
\end{minipage}
\end{tcolorbox}
...
\begin{center}\parbox{0.85\textwidth}{\color{red!60!black}
...
augment the value and accuracy of research 
data by provisioning scientists with digital 
tools to find, evaluate, and reuse data 
which is relevant to their research.  
At the present moment, the urgency of sharing \Covid{} 
research is further boosting this data-centric and open-access publishing model.  Indeed, \Covid{} 
...
(that of coronaviruses in general and \Covid{} in particular.}
...
perspective on the curation and structuration of 
...
White House initiative devoted to \Covid{} (see 
\href{https://www.whitehouse.gov/briefings-statements/call-action-tech-community-new-machine-readable-covid-19-dataset/}{https://www.whitehouse.gov/briefings-statements/call-action-tech-community-new-machine-readable-covid-19-dataset/}).  In particular, 
...
\p{The sudden emergence of \Covid{} as a 
...
have, to some degree, recognized the extraordinary 
nature of the new coronavirus crisis and have 
...
biologically similar to \Covid{}).  Then, 
on March 16, the White House announced the 
project referenced earlier, describing this 
initiative as a 
...
publication text  
for over 44,000 coronavirus research papers.  
...
texts relevant to \Covid{}.  The \Cnineteen{} 
...
published: \q{The White House joins these institutions in issuing a call to action to the Nation's artificial intelligence experts to develop new text and data mining techniques that can help the science community answer high-priority scientific questions related to COVID-19.}  
In short, \Cnineteen{} was released with the 
...
curated \Cnineteen{}, enourages publishers to contribute new 
...
as just outlined).  These problems can be ameliorated 
by equipping publishers with sotfware that can construct 
infosets concordant to the \Cnineteen{} schema from 
their composit-stage documents; we can provide this software 
as part of the proposed \SDK{}.}

\p{As editor of the International Journal of Speech Technology, 
(\IJST{}, published by Springer) 
as well as \textit{Advances in Ubiquitous Computing}, I am 
trying to promote this form of technology by implementing 
similar tools for these publications.  In particular, 
we have developed a text-encoding library which will 
allow the text of \textit{Advances} to be published in a 
machine-readable format consistent with the 
\Cnineteen{} schema; and similar technology can 
be used for future editions of \IJST{} as well 
as subsequent volumes in the \textit{Advances in Ubiquitous 
Sensing for Healthcare} series.  It is certainly possible 
that some future articles in this series will be 
relevant to \Covid{}; indeed, remote diagnosis and 
cyber-physical screening devices (both of which are 
intrinsic to the field of medical Ubiquitous Sensing) 
are an important 
aspect of attempts to mitigate the pandemic.  I 
am hoping that the corpus-integration technology I 
adopt as an editor could provide a prototype which 
can be adapted by journals that may feed articles 
directly to \Cnineteen{} and similar initiatives.  
In particular, the process of adding new publications 
to resources such as Springer Nature should automatically 
include the operation of generating machine-readable 
text or metadata which can be integrated with 
corpora such as \Cnineteen{}.  As archival operations 
are more tightly supported within a publishing 
pipeline, problems such as those outlined above 
are less likely to emerge.  Concomitantly, 
the problems discussed in the following 
paragraphs are due in part to the logistical 
\textit{gap} between publishers and archival 
curates (e.g., in the case of \Cnineteen{}, the 
Allen Institute).} 
...
transcription errors (especially in relation to 
technical or scientific phrases and terminology); 
scientific notation in particular may be improperly 
encoded.  A formal designation such as \colorq{2{\textquotesingle}-C-ethynyl}
(to take one specific case) is encoded in \Cnineteen{} 
as \colorq{2 0 -C-ethynyl}, which could stymie text searches 
against the \Cnineteen{} corpus (see \cite{Eyer}).  Moreover, there is no 
semantic marking identifying that the \colorq{2 0 -C-ethynyl} 
...
merging and integrating \Covid{} data from 
...
\p{Aside from the issues (described in the preceding 
section) which are likely to hinder text and data mining 
...
consequently, our proposed \SDK{} would include a custom 
data-annotation library, to be employed in conjunction with 
operations to aggregate \Cnineteen{} data sets into a 
common representational format.  In addition, 
the \SDK{} would include a customized document viewer 
which embeds code to recognize and interact with 
microcitations in both manuscript and data-set environments.}
...
context.  By including a self-contained computing 
framework as an integral package accompanying 
...
WhiteDB for the database kernel (see \cite{EnarReilent}), AngelScript 
for the scripting kernel (see \cite{AS}), and \Qt{} 
(see \cite{IvanCukic}) for 
...
\p{Having described an \MThreeK{} model for \textit{individual} 
...
extending an \MThreeK{} \SDK{} with domain-specific 
...
of \Covid{} research, from demographic and 
...
and with other \Covid{} dashboards and data sets.   
...
apply to other \Covid{} related resources.  For 
example, John Hopkins University has created and deployed 
a \Covid{} \q{dashboard} tracking the spread of the virus 
...
when the aforementioned Verily portal comes online, hopefully machine-readable access 
to that public data will be 
provided either via an analogous updated archive or 
via an \API{}.  Ideally, these disparate \Covid{} 
...
In particular, a useful starting-point for \Covid{} 
...
\Covid{} would serve as a first example and a test-bed 
for this new paradigm, given the potential public 
benefit of unifying disparate \Covid{} data 
...
\Covid{} archive that could serve as a model 
