...
reflect publishers' current prioritization 
of publicly shared research data.  Implementing 
this new \q{data-centric} publishing paradigm 
requires more than just enouraging authors to 
prepare downloadable data sets accompanying research 
papers; it also requires a technological 
infrastructure to support the curation of 
data sets and data set collections, so that 
scientists can locate data sets which are 
relevant to their research and then incorporate these
data sets into subsequent research.  One goal 
of \textit{Advances in Ubiquitous Computing} 
...
we propose to create a Standard Development Toolkit 
...
software.  Our porposed \Cnineteen{} \SDK{} would be 
one example of a component providing analytic 
capabilities which make the raw \Cnineteen{} data 
more valuable, and also a toolkit for creating 
...
in the connection between papers archived 
by \Cnineteen{} and Covid-19 research data.  
With respect to text mining, an immediate problem lies 
in how the text was parsed from \PDF{} files, a process 
which can cause imprecise or inaccurate text representation.  
In particular, the \Cnineteen{} library evinces  
