...
{Proposing a CORD-19 SDK Solution to Improve Machine Readability \\\vspace{6pt}
 {\fontfamily{ptm}\fontsize{12}{14}\selectfont to coordinate with \\SPRINGER NATURE, WILEY, ELSEVIER,\\the AMERICAN SOCIETY FOR MICROBIOLOGY, 
...
library, which could be provided as part of a 
\Cnineteen{} \SDK{}, could implement convenience procedures to handle 
recurring tasks, such as obtaining the full bibliographic 
reference affixed to a given location in a manuscript.}

\p{A further complication in \Cnineteen{} arises 
from how the text was parsed from \PDF{} files, a process 
which can cause imprecise or inaccurate text representation.  Upon downloading and 
examining the \Cnineteen{} library, one will encounter 
transcription errors arising from how document  
   
For example, scientific notation may be improperly 
transcribed; a formal notation such as \q{2{\textquotesingle}-C-ethynyl}
...
formats such as \XML{}.  To address these problems, 
it would be optimal to cross-reference the 
...
access to publications in multiple formats.  In addressing this problem, our proposed \SDK{} for 
\Cnineteen{} would include built-in client libraries 
with procedures to 
merge different representations of each document.  As well 
as facilitating text-mining across the corpus, these client 
libraries would also improve the usability of \Cnineteen{} data sets; 
in particular, it would be possible to search the document corpus 
for terms which appear in data sets, thereby building a 
network of links between data sets and publications.}
...
Research Objects --- the one which would be adopted for our proposed \Cnineteen{} \SDK{} --- is to provide a kind of out-of-the-box 
