...
will accompany the volume \textit{Cross-Disciplinary Data 
Integration Models for the Emerging Covid-19 Data 
Ecosystem}, scheduled to be published in October of 
this year (2020).  The current paper will discuss 
the goals and structure of \CRtwo{}.  It will also 
explain how scientists or oganizations can contribute 
...
the corpus is not prohibitively large.  Having 
said that, we believe a more substantial 
(and not necessarily fully open-access) Covid-19 
information space would be beneficial to the 
scientific community.  Ideally, \CRtwo{} can be 
...
columns in one table correspong semantically or conceptually 
to those in a second table.  In either case, 
it is reaonable to assume that a single abstract data 
format lies behing surface data-expression in forms 
like spreadsheets and \CSV{}, so that all files in an 
archive encoding spreadsheet-like data can be 
migrated to a common model.  Other forms of 
...
into the team's desicion to \textit{administer} some 
treament (say, a drug \textit{from} some provider 
...
models as outlined in \cite{EranBellin}, for example.}     
...
works with a meta-model which incoporates both 
...
paradigm, based on the HyperGraphDB database engine, 
to merge many different sorts of information into a 
...
technogically achieves its stated goals.  The 
\HGXF{} format will therefore be examined 
in greater detail in the following section.  
Researchers who are considering submitting data 
to \CRtwo{} should bear in mind that we will 
attempt to convert files comprising the related 
data sets into \HGXF{} format, in whole or in 
part, and to provide \HGXF{} files as proxies 
to resources in other formats as needed.}
...
(\cite{EnarReilent}), and HgLib (\cite[p. 9]{Erable}).  That is, 
...
and software, with repect to metadata, \GUI{} integration, 
...
\item[From the Ground Up \Qt{} Integration]  All 
\DgDb{} classes natively interoperate with 
\Qt{} (a leading cross-platform application-developement 
...
microcitations, where must be some formal 
...
databas  atom, information that could be used for debugging, storing application state, inter-application networking (e.g. linking \PDF{} viewers for research papers with applications to view research data), 
and so forth.
...
(see \cite[Chapter 3]{NeusteinCPS}) that may, 
...
\p{The prior outline decribed some of the principle 
features of \DgDb{}, which in turn influened 
...
context (i.e., edges are asserted relations 
that apply in general, with no 
...
be defined within a controled vocabulary) and/or 
...
this construction as well, essentially constructing 
hypernodes \q{on the fly}, but in this case the 
fields within these auto-constructed hypernodes are 
\q{proxies} for hypernodes (possibly three or more) 
...
desribing research data.  Finally, \lDgDb{} is a 
...
manuscripts as well as the collection of research 
