...
\geometry{left=.73in,top=.9in,right=.52in,bottom=1.4in} %margins

\renewcommand{\thepage}{\raisebox{-1em}{\arabic{page}}}
...
\newcommand{\leXML}{\resizebox{!}{7pt}{\AcronymText{leXML}}}
...
and Text Mining Interoperability}
}}}
\end{center}
\end{minipage}
\end{tcolorbox}
...
\LVee{}The basic foundation of this \SDK{} would 
be a framework called Annotation Exchange Format 
(\AXF{}), which LTS designed to facilitate 
document preparation as well as interoperability 
between text and/or data mining tools.  
\lAXF{} is employed as a querying format 
integrated with data sets accompanying the 
forthcoming \textit{Advances in Ubiquitous Computing} 
volume, part of Elsevier's \textit{Ubiquitous 
Sensing for Healthcare} series.\LWedge{}  
Our proposed \SDK{} 
...
Producing annotations as part of the document-preparation 
process represents only a small extension to the tasks 
of authoring and compositing documents to begin with.  
...
are \textit{not} punctuation markers have a distinct 
...
authoring or editing paradigms can be applied 
which generate highly structured documents 
with built-in sentence and Named Entity demarcations.}

\p{Of course, text-mining technology is needed 
for the relatively less-well-structured publications 
which constitute most of our digital ecosystem.  
The \AXF{} format, however, is built around data 
...
content).  With this genre of annotation --- produced 
directly from pre-publication manuscripts --- as a 
foundation, \AXF{} then generalizes to include 
notation related to text and data mining \API{}s and 
file format.  For example, an \AXF{} representation 
...
\leXML{}, \CoNLLU{}, OpenAnnotations, and \SciXML{}.  
Moreover, \AXF{} can certainly evolve 
to support new capabilities or resources, 
based on the practical requirements of projects 
such as \Cnineteen{}.}

\p{For document preparation, \AXF{} is 
paired with a Hypergraph Text Encoding Protocol 
(\HTXN{}), which provides a canonical character 
encoding against which annotations can be 
defined.  An annotation target is therefore a 
character-index interval in the context of an 
\HTXN{} character stream.  \lHTXN{} code-points 
...
or part of an ellipsis.  On that basis, 
...
and end).  Procedures are implemented to 
...
(including \ASCII{}) is considered a derived 
property of the annotation, not a foundational 
representation.}

\p{Both \AFX{} and \HTXN{} are integrated with 
...
containing \AFX{} and \HTXN{} data, using this 
information to context menus sensitive to the 
...
manuscripts for \AFX{} data.  These 
