...
\titlespacing*{\section}
{0pt}{4.5ex plus 1ex minus .5ex}{.7ex plus .2ex}
...
The \AXF{} framework includes an \SDK{} oriented to 
the construction of document archives organized 
...
\AXF{} \SDK{}.  Our proposed \Cnineteen{} \SDK{} 
...
where the \API{} may be valuable for obtaining 
information such as Semantic Scholar paper ids 
and entity strings, which can be employed when 
building metadata files embedded in publications and/or 
to add features to human-readable manuscripts 
(examples in the second case include providing 
Semantic Scholar paper ids within bibliographic 
...
paragraph ids, and character indices.  Consequently, 
the archival schema's mechanism for standoff 
annotation of intra-document references may be 
extended for assertions of linguistic, resource,  
or topical annotations from multiple Annotation Spaces.  
The proposed \SDK{}, in particular, 
...
detail intermediate between linked-data-oriented 
...
dependency relations), although it does support 
...
a research \textit{archive}.  Analogous to code bundled 
with a single research \textit{object}, an archive-specific 
\SDK{} is then associated with each Research Archive.  In 
the case of \Cnineteen{}, the archive \SDK{} would serve the 
goal of integrating 
