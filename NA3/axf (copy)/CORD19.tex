...
is a new coronavirus data collection which was released 
in conjunction with a White House initiative to 
...
\LVee{}The foundation of the \SDK{} is 
a framework that we call \q{Annotation Exchange Format} 
(\AXF{}), which is designed to facilitate 
...
which is part of Elsevier's Series, \textit{Advances in Ubiquitous 
Sensing Applications for Healthcare}.\LWedge{}  
The \AXF{} framework includes an \SDK{} geared to 
the construction of document archives that are organized 
around particular themes or topics; our proposed 
\SDK{} for \Cnineteen{} would therefore be a 
specialization and extension of our generic 
Annotation Exchange Framework \SDK{}.  Our proposed 
\Cnineteen{} \SDK{} would include new code libraries explicitly 
implemented for data-management operations 
specific to \Cnineteen{}.  It would also include a package 
of applications, modified to support \Covid{} research, 
that would collectively create an integrated 
and self-contained computing environment.  
These three parts of the \SDK{} --- 
...
%\vspace{-2em}
\begin{description}[leftmargin=9pt,itemsep=2pt]

\item[Datatypes Encapsulating Access to the 
Semantic Scholar API]  This library would provide 
a \Qt{}-based wrapper around Semantic Scholar \API{} 
calls and model certain objects relevant to this 
\API{}, such as entities and their mentions, 
citations, and authorship links.  One use-case 
for such a library concerns document preparation. 
Here the \API{} may be valuable for obtaining 
information such as Semantic Scholar paper ids 
and entity strings, which can be employed when 
building metadata files embedded in publications; 
and/or to add features to human-readable manuscripts 
(e.g., providing Semantic Scholar 
paper ids within bibliographic 
entries, and selecting keywords based on entities 
mentioned within cited papers).   
...
\JSON{} representation.  
The \SDK{} would then provide tools to 
...
\item[An LAF Architecture for Annotations over CORD-19]
The structure of \Cnineteen{} allows text segments to 
be defined via a combination of \JSON{} file names, 
paragraph ids, and character indices.  
With suitable programming, this structure 
may be utilized as a concrete instantiations of 
the \LAF{} (Linguistic Annotation Framework) data model 
\cite{IdeSuderman}, 
so that the archival schema's standoff 
annotation mechanism can be 
extended for assertions of linguistic, resource,  
or topical annotations from multiple Annotation Spaces.  
Our proposed \SDK{}, in particular, 
would provide code for additional 
annotation categories of the kinds 
...
\LVee{}
\section{The Annotation Exchange Format (AXF)}
\p{Conceptually, the \AXF{} system 
is intended for document preparation as well as for text 
mining.  As a rule, annotations deliberately introduced 
...
documents can additionally serve as guidelines for 
...
(as well as quotations, citations,\footnote{%
Preprocessing docouments allows fairly detailed 
citation info to be compiled.  For instance, 
Semantic Scholar or Crossref paper ids can be listed 
in bibliographies, and citation intent and context 
can be asserted (in the case of intent, this feature 
is enabled via a modified \texttt{\textbackslash{}cite} command 
with an option for authors to note the relation of the 
cited work to the current document.%
}hyperref links, equations, and 
...
In particular, the \AXF{} toolkit includes a command-line 
tool to \q{query} manuscripts using an 
interface based on BeCAS.  Moreover, \AXF{} 
uses a two-tier node structure similar to 
...
requirements.  Moreover, \AXF{} can certainly expand 
...
location of that text which is an annotation 
...
text to the clipboard (where the relevant 
...
translate it to a larger data space, viz., 
a research \textit{archive}.  An archive-specific 
\SDK{} is then associated with each research archive.  
These \SDK{}s are analogous to code bundled with a single 
research \textit{object}, but with an expanded 
focus, one that encompasses the archive as a whole.  
In the case of 
\Cnineteen{}, the archive \SDK{} would serve the 
goal of integrating the information contained in multiple 
