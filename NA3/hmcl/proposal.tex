...
\newcommand{\DICOMRT}{\resizebox{!}{7pt}{\AcronymText{DICOM-RT}}}
...
and diagnostic imaging.  Patient-centered research in 
...
reports, provide layers data that exist prior to the initiation 
...
\p{Unfortunately, as it is presently constituted, the 
research ecosystem for developing novel 
medical-imaging techniques is oriented 
toward diagnostic precision, rather than 
toward treatment recommendations.  For 
example, there are numerous data sets commonly 
used by researchers to evaluate and demonstrate 
novel image analytic techniques, many of which 
include some rudimentary clinical data, but 
few or none of which document post-diagnostic 
treatment outcomes.  Moreover, most of the software 
packages and applications used to perform 
image analysis do not support data types 
for representing patient outcomes as data 
structures that can be analyzed and 
cross-referenced with other sorts of data.  
When an image or image series is 
analyzed --- typically resulting in a new 
image, image annotation, and/or a feature 
vector providing a statistical quantification 
of the image's content --- there is no 
established protocol for appending this 
new analytic data to prior records concerning 
the patient's diagnosis, treatment, and outcomes.  
In short, even if medical institutions were 
to agree that integrating clinical outcomes 
models (including patient-centered models 
whose parameters are not only survival rates 
but also quality-of-life oriented) is a worthy 
goal, the capacity to generate data enabling 
such an initiative would need to be inserted 
into existing diagnostic-imaging software.}

\p{The group of contributors submitting the 
current proposal is not the first to 
identify these problems, and several 
projects have addressed the need to 
cross-reference clinical/outcomes data 
with diagnostic imaging.  The \PROD{} 
study can be counted among this group, 
because it demonstrates that experiential 
...
\p{Although \SeDI{} defines a general-purpose 
ontology --- essentially, a system for mapping 
\DICOM{} header data to \RDF{} structures so as 
to overcome inconsistencies in different 
\DICOM{} files which can inhibit query evaluation 
across \DICOM{} corpora --- its concrete 
realization is tied to a specific commercial 
\PACS{} technology.  The \ViSion{} framework, 
whose goals are fairly similar to \SeDI{} 
(albeit with further emphasis on \q{structured 
reporting,} including via Natural Language 
Processing) similarly pairs a general-purpose 
ontology with one specific medical software product.  
Insofar as the \ViSion{} and \SeDI{} projects 
are the two most detailed \q{semantic \DICOM{}} 
efforts discussed in recent literature, 
it seems reasonable to conclude that the idea of 
a semantic and \q{searchable} \PACS{} system, 
and image-repository framework, is not yet 
part of the diagnostic-imaging ecosystem in 
any foundational sense (notwithstanding features 
that might be present in a small group of 
specialized commercial products).}

\p{An additional, and related, limitation of 
existing medical-imaging software concerns 
the integration of novel image-analysis 
methodology.  Typical diagnostic-imaging 
software includes preliminary image-processing 
capabilities --- acquiring and parsing \DICOM{} 
files, for instance, and converting \DICOM{} 
into formats such as \textbf{itk::Image} 
for analysis --- and some inventory 
of image analysis and transformation 
functionality.  Because diagnostic imaging is a 
very active field of research, new techniques 
and algorithms are constantly being published, 
which creates a challenge of integrating new 
methodology into pre-existing systems.  
Novel analyses, for their part, may be implemented 
in many different programming contexts, such 
as \R{}, Jupyter (which is a python-based 
development environment), Matlab, or standalone 
libraries/applications written in languages such 
as Python, Java, or \Cpp{}.  This diversity 
of development environments can make it difficult 
to \q{mix-and-match} algorithms whose implementation 
depends on a specific ecosystem --- for instance, 
to run a \PACS{} workstation implemented in 
Java but utilize an analytic process implemented 
in \R{}.  Such ecosystem-level mialignment is 
generally mitigated by employing some form of 
workflow description, such as the 
Common Workflow Language (\CWL{}), 
which allows independent software components 
to interoperate even if they are implemented 
in fundamentally incompatible programming 
frameworks.}
...
extensions.\footnote{No native \C{} or \Cpp{} 
libraries are described on the \CWL{} website 
among the tools and parsers available for \CWL{}, 
but \CaPTk{} is mentioned on a corresponding discussion 
thread concerning \Cpp{} libraries.  It seems 
therefore that the \CaPTk{} \q{utilities} repository 
provides the de-facto standard \Cpp{} implementation of 
\CWL{}, at least according to the \CWL{} group 
...
intergrated into the total space of data 
...
of current diagnostic imaging softare: 
...
by these two projects.  This is, indeed, 
one component of the new platform which we 
propose to implement as part of the current 
proposal.  On a broader level, however, 
the concrete examples of \SeDI{} and \CaPTk{} 
point to    The integrative logic of 
...
extended \DICOM{} to incoporate radiation-therapy 
