 ...
 switch(unit_choice)
{
 ...
 qts_write("at least <b><font color=#357EC7>%g %s</font></b>", value, unit);
 ...
 switch(unit_choice)
{
 ...
 qts_write("at most <b><font color=#357EC7>%g %s</font></b>", value, unit);
 if((option_no == 2) && (options_flag == 1))
   max_time_value = value * mul;
 else if(option_no > 2)
{
 ...
 switch(unit_choice)
{
 ...
 qts_write("~<b><font color=#357EC7>%g %s</font></b>", value, unit);
 if((option_no == 2) && (options_flag == 1))
   max_time_value = value * mul;
 else if(option_no > 2)
{
...
//class Speed
Speed::Speed(r4 x, enum speed_type unit)
{
 value = x; 
 unit_choice = unit;
}
...
Minimum_speed::Minimum_speed (r4 x, speed_type unit)
{
 value = x; 
 unit_choice = unit;
}
 ...
 switch(unit_choice)
{
 ...
 qts_write("a speed of at least <font color=#357EC7>%g %s</font>", value, unit);
}
 ...
 switch(unit_choice)
{
 ...
 qts_write("a speed of at most <font color=#357EC7>%g %s</font>", value, unit); 
}
 ...
 switch(unit_choice)
{
 ...
 qts_write("a speed of ~<font color=#357EC7>%g %s</font>", value, unit); 
}
...
//class Volume
Volume::Volume(r4 x, enum vol_unit unit1)
{
 value = x; 
 unit_choice = unit1;
 switch(unit_choice)
...
void Symbolic_vol:: display_vol()
{
 Volume* vol1;
 if (s1.value == DEFAULT)
  fprintf(fp,"<b><font color=#357EC7>%s mcL</font></b>", s1.symbol); 
 else 
{
...
Volume_range::Volume_range(r4 x, r4 y, enum vol_unit unit1)
{
 value1 = x;
 value = y;
 unit_choice = unit1;
...
{
 qts_write("<b><font color=#357EC7>%g - %g %s</font></b>", value1, value, unit);
}
Approx_volume::Approx_volume(r4 x, enum vol_unit unit1)
{
 value = x; 
 unit_choice = unit1;
 switch(unit_choice){
...
Time* time (r4 x, enum time_unit unit)
{
 Time *temp = new Time(x, unit);
 return temp;
}
Time_range* time_range (r4 x, r4 y, enum time_unit unit)
{
 Time_range *temp = new Time_range(x, y, unit);
 return temp;
}

Minimum_time* min_time(r4 x, enum time_unit unit)
{
 Minimum_time *temp = new Minimum_time(x, unit);
 return temp;
}
Maximum_time* max_time(r4 x, enum time_unit unit)
{
 Maximum_time *temp = new Maximum_time(x, unit);
 return temp;
}
Approx_time* approx_time(r4 x, enum time_unit unit)
{
...
Speed_range* speed_range(r4 x, r4 y, enum speed_type unit)
{
 Speed_range *temp = new Speed_range(x, y, unit);
 return temp;
}

Minimum_speed* min_speed(r4 x, enum speed_type unit)
{
 Minimum_speed *temp = new Minimum_speed(x, unit);
 return temp;
}
Maximum_speed* max_speed(r4 x, enum speed_type unit)
{
 Maximum_speed *temp = new Maximum_speed(x, unit);
 return temp;
}
Approx_speed* approx_speed(r4 x, enum speed_type unit)
{
 Approx_speed* temp = new Approx_speed(x, unit);
 return temp;
}

Volume* vol(r4 x, enum vol_unit unit)
{
...
Volume_range* vol_range(r4 x, r4 y, enum vol_unit unit)
{
 Volume_range* result = new Volume_range(x, y, unit);
 return result;
}
Approx_volume* approx_vol(r4 x, enum vol_unit unit)
{
...
void start_protocol(char* name)
{
 ...
 fp = fopen(temp1, "w");
 qts_write("<h1 style=\"font-size = 25px;\">%s</h1>", name);
}
...
void first_step(char* step_name)
{
 first = 1;
 qts_write("</ul><h2>Steps:</h2><ol>");
 qts_write("<p><li><b><font size=3>%s</font></b><br>", step_name);
}
 ...
 if (sub_step_flag == 1)
{
 ...
 qts_write("</li></p><p><li><b><font size=3>%s</font></b><br>", step_name);
}
 ...
 if (sub_step_flag == 1)
{
 ...
 qts_write("</li></p><p><b><font size=3>%s (Optional)</font></b><br>", name);
}
...
{
 qts_write("Repeat Step %d. <br>", step_no);
}
...
void display_equip()
{
 ...
 for(i=0; i<equip_no; i++)
  qts_write("<li>%s</li>", equipments[i]);
 for(i=0; i<cont_no; i++)
  qts_write("<li>%s</li>", containers[i]);
 qts_write("</ul></div>");
}
...
void usage_details()
{
 ...
 for(i=0;i<list_fluid_no;i++)
 {
  ...
  if (count1 == 0)
  {
   ...
   qts_write("<li>%s</li>", usage_list_fluids[i].original_name);
   count1++;
  }
  else
   qts_write("<li>%s</li>", usage_list_fluids[i].original_name);
 ...
 for(i=0;i<list_container_no;i++)
 {
  ...
  if (count2 == 0)
  {
   ...
   qts_write("<li>%s</li>", usage_list_containers[i].name);
   count1++;
  }
  else
   qts_write("<li>%s</li>", usage_list_containers[i].name);
...
void timing_details()
{
 ...
 if (time_hrs == 0)
  qts_write("<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d mins</font></b></p>", time_mins);
 else if (time_hrs == 1)
  qts_write("<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d hr, %d mins</font></b></p>", time_hrs, time_mins);
 else
  qts_write("<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d hrs, %d mins</font></b></p>", time_hrs, time_mins);
...
void check_container(Container& container1)
{
 if ((first == 1) || (prev_cont == 1)||(prev_container != container1.name))
 {
  ...
  qts_write("%s", container1.name);
 }
}

void to_do(char* clarify)
{
 qts_write("<font color = \"red\"><i>%s</i></font><br>", clarify);
}

void comment(char* comm)
{
 qts_write("<font color = \"#800517\"><i>%s</i></font><br>", comm);
}
...
{ 
 ...
 if(temp == ICE_COLD)
 {
  qts_write("<li>ice-cold %s</li>", name, temp);
  ...
  qts_write("<li>%s stored at room temperature</li>", name);
 else
  qts_write("<li>%s stored at %g%cC</li>", name, temp, 0x00B0);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
Fluid new_fluid(char*name, char* state)
{
 ...
 qts_write("<li> <a name=\"%s\">%s <i><br><tab><div style=\"margin-right: 600px;\">(%s)</div></i></a></li>", name, name, state);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
Fluid new_fluid (char* name, char* state, r4 temp)
{
 ...
 if(temp == ICE_COLD)
 {
  ...
  strcat(t1, name);
  result.new_name = t1;
  qts_write("<li> <a name=\"%s\"> %s <i><br><tab><div style=\"margin-right: 600px;\">(%s)</div></i></a></li>", result.new_name, result.new_name, state);
 }
 else if(temp >= 35)
   qts_write("<li> <a name=\"%s\">%s preheated in a water bath set at <b><font color=#357EC7>%g%cC</font></b> <i>%s</i></a></li>", name, name, temp, 0x00B0, state);
 else
   qts_write("<li> <a name=\"%s\">%s <i>%s</i> </a> at <b><font color=#357EC7>%g%cC</font></b></li>", name, name, state, temp, 0x00B0);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
{ 
 ...
 if(temp == ICE_COLD)
 {
  ...
  qts_write("<li>%s at %g%cC</li>", name, temp, 0x00B0);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
Solid new_solid(char*name, char* state)
{
 ...
 qts_write("<li> <a name=\"%s\">%s <i><br><tab>(%s)<br></i></a></li>", name, name, state);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
Solid new_solid (char* name, char*state, r4 temp)
{
 ...
 if(temp == ICE_COLD)
 {
  qts_write("<li> <a name=\"%s\">ice-cold %s <i><br><tab>(%s)<br></i></a></li>", name, name, state);
  t1 = (char *)calloc(strlen(t1) + strlen(name) + 10, sizeof(char));
  strcat(t1, "ice-cold ");
  strcat(t1, name);
  result.new_name = t1;
...
void measure_fluid(Fluid& fluid1, Container& container1)
{
 // graph maintenance
 {
  ...
  if (fluid1.volume == 0)
   qts_write("<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", fluid1.new_name);
  if (usage_list_fluids[fluid1.usage_index].original_name == fluid1.original_name)
   usage_list_fluids[fluid1.usage_index].used = 1;
  else 
  {
  ...
  if (first == 1)
  {
   ...
   {
    if(fluid1.state == "")
     qts_write("Measure out %s into %s.<br>", fluid1.new_name, container1.name);
    else
     qts_write("Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.name);
    first = 0;
    prev_cont++;
   }
   else
   {
    if(fluid1.state == "")
     qts_write("Measure out %s into %s.<br>", fluid1.new_name, container1.contents.new_name);
    else
     qts_write("Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.contents.new_name);
    first = 0;
    prev_cont++;
   }
  }
  ...
  {
   qts_write("Add <font color=#357EC7>%s</font> to %s.<br>", fluid1.new_name, container1.contents.new_name);
   prev_container = container1.name;
  }
...
void measure_fluid(Container& container, Container& container1)
{
 // graph maintenance
 {
  ...
  if (container.volume == 0)
   qts_write("<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", container.contents.new_name);
  ...
  if (first == 1)
  {
   ...
   if (container1.contents.new_name == "")
   {
    if(container.contents.state == "")
     qts_write("Measure out %s into %s.<br>", container.contents.new_name, container1.name);
    else
     qts_write("Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", container.contents.new_name, container.contents.new_name, container1.name);
    first = 0;
    prev_cont++;
   }
   else
   {
    if(container.contents.state == "")
     qts_write("Measure out %s into %s.<br>", container.contents.new_name, container1.contents.new_name);
    else
     qts_write("Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", container.contents.new_name, container.contents.new_name, container1.contents.new_name);
    first = 0;
    prev_cont++;
   }
  }
  ...
  {
   qts_write("Add <font color=#357EC7>%s</font> to %s.<br>", container.contents.new_name, container1.contents.new_name);
   prev_container = container1.name;
  }
...
void measure_fluid(Fluid& fluid1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  volume1->display_vol();
  if(fluid1.state == "")
   qts_write(" of %s.<br>", fluid1.new_name);
  else
   qts_write(" of <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", fluid1.new_name, fluid1.new_name);
...
void measure_fluid(Fluid &fluid1, Volume* volume1, Container& container1)
{
 // graph maintenance
 {
  ...
  if (first == 1)
  {
   ...
   {
    if(fluid1.state == "")
     qts_write(" of <font color=#357EC7>%s</font> into %s.<br>", fluid1.new_name, container1.name);
    else
     qts_write(" of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.name);
    first = 0;
    prev_cont++;
   }
   else
   {
    if(fluid1.state == "")
     qts_write(" of <font color=#357EC7>%s</font> into %s.<br>", fluid1.new_name, container1.contents.new_name);
    else
     qts_write(" of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.contents.new_name);
    first = 0;
    prev_cont++;
   }
  }
  ...
  else if (prev_container == container1.name)
  {
   ...
   if(fluid1.state == "")
   {
    qts_write("Add ");
    volume1->display_vol();
    qts_write(" of <font color=#357EC7>%s</font>.<br>", fluid1.new_name);
   }
   else
   {
    qts_write("Add ");
...
void measure_fluid(Container& con, Volume* volume1, Container& container1)
{
 // graph maintenance
 {
  ...
  if ((con.volume == 0)|| (con.volume < (volume1->value*volume1->mul)))
   qts_write("<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", con.contents.new_name);
  ...
  if (first == 1)
  {
   ...
   if (container1.contents.new_name == "")
   {
    if(con.contents.state == "")
     qts_write(" of <font color=#357EC7>%s</font> into %s.<br>", con.contents.new_name, container1.name);
    else
     qts_write(" of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", con.contents.new_name, con.contents.new_name, container1.name);
    first = 0;
    prev_cont++;
   }
   else
   {
    if(con.contents.state == "")
     qts_write(" of <font color=#357EC7>%s</font> into %s.<br>", con.contents.new_name, container1.contents.new_name);
    else
     qts_write(" of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", con.contents.new_name, con.contents.new_name, container1.contents.new_name);
    first = 0;
    prev_cont++;
   }
  }
  ...
  else if (prev_container == container1.name)
  {
   ...
   if(con.contents.state == "")
   {
    qts_write("Add ");
    volume1->display_vol();
    qts_write(" of <font color=#357EC7>%s</font>.<br>", con.contents.new_name);
   }
   else
   {
    qts_write("Add ");
    volume1->display_vol();
    qts_write(" of <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", con.contents.new_name, con.contents.new_name);
   }
  }
  else
  {
...
void set_temp(Container& container1, r4 temp)
{
 // graph maintenance
 {
  ...
  if (temp >= 35)
   qts_write("Pre-heat %s in a water bath set at <b><font color=#357EC7>65%cC</font></b>.<br>", container1.contents.new_name, 0x00B0);
  else
   qts_write("Set the temperature of %s to <b><font color=#357EC7>%g%cC</font></b>.<br>", container1.contents.new_name, temp, 0x00B0);
}
...
void measure_prop(Container& dest, Container& source, r4 prop)
{
 // graph maintenance
 {
  ...
  if ((source.contents.volume == 0)|| (source.contents.volume < (dest.volume * prop)))
   qts_write("<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", source.contents.new_name);
  ...
  if (first == 1)
  {
   ...
   {
    if (prop > 1)
     qts_write("Measure out <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font> into %s.<br>", prop, source.contents.new_name, dest.contents.new_name);
    else
     qts_write("Measure out <b><font color=#357EC7>%g</font></b> volume <font color=#357EC7>%s</font> into %s.<br>", prop, source.contents.new_name, dest.contents.new_name);
   }
   else
   {
    if (prop > 1)
     qts_write("Measure out <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", prop, source.contents.new_name, source.contents.new_name, dest.contents.new_name);
    else
     qts_write("Measure out <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", prop, source.contents.new_name, source.contents.new_name, dest.contents.new_name);
   }
  }
  else if (prev_container == dest.name)
  {
   ...
   {
    if (prop > 1)
     qts_write("Add <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", prop, source.contents.new_name, source.contents.new_name);
    else 
     qts_write("Add <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", prop, source.contents.new_name, source.contents.new_name);
   }
   else 
   {
    if (prop > 1)
     qts_write("Add  <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font>.<br>", prop, source.contents.new_name);
    else
  ...
  else
  {
   ...
   {
    if (prop > 1)
     qts_write("Add <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", prop, source.contents.new_name, source.contents.new_name, dest.name, dest.name);
    else 
     qts_write("Add <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", prop, source.contents.new_name, source.contents.new_name, dest.name);
   }
   else 
   {
    if (prop > 1)
     qts_write("Add  <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font> to %s.<br>", prop, source.contents.new_name, dest.name);
    else
...
void measure_prop(Container& dest, Fluid& source, r4 prop)
{
 // graph maintenance
 {
  ...
  if ((source.volume == 0)|| (source.volume < (dest.volume*prop)))
   qts_write("<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", source.new_name);
  ...
  if (first == 1)
  {
   ...
   {
    if (prop > 1)
     qts_write("Measure out <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font> into %s.<br>", prop, source.new_name, dest.contents.new_name);
    else
     qts_write("Measure out <b><font color=#357EC7>%g</font></b> volume <font color=#357EC7>%s</font> into %s.<br>", prop, source.new_name, dest.contents.new_name);
   }
   else
   {
    if (prop > 1)
     qts_write("Measure out <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", prop, source.new_name, source.new_name, dest.contents.new_name);
    else
     qts_write("Measure out <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", prop, source.new_name, source.new_name, dest.contents.new_name);
   }
  }
  else if (prev_container == dest.name)
  {
   ...
   {
    if (prop > 1)
     qts_write("Add <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", prop, source.new_name, source.new_name);
    else 
     qts_write("Add <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", prop, source.new_name, source.new_name);
   }
   else 
   {
    if (prop > 1)
     qts_write("Add  <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font>.<br>", prop, source.new_name);
    else
     qts_write("Add  <b><font color=#357EC7>%g</font></b> volume <font color=#357EC7>%s</font>.<br>", prop, source.new_name);
   }
  }
  else
  {
   ...
   {
    if (prop > 1)
     qts_write("Add <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", prop, source.new_name, source.new_name, dest.name);
    else 
     qts_write("Add <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", prop, source.new_name, source.new_name, dest.name);
   }
   else 
   {
    if (prop > 1)
     qts_write("Add  <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font> to %s.<br>", prop, source.new_name, dest.name);
    else
     qts_write("Add  <b><font color=#357EC7>%g</font></b> volume <font color=#357EC7>%s</font> to %s.<br>", prop, source.new_name, dest.name);
   }
  }
...
void transfer (Container &container1, Container& container2)
{
 // graph maintenance
 {
  ...
  if (container1.volume == 0)
   qts_write("<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", container1.contents.new_name);
  ...
  qts_write("Transfer %s into %s.<br>" , container1.contents.new_name, container2.name);
...
void centrifuge_pellet(Container& container1, Speed* speed1, r4 temp, Time* time1, Volume* expected_vol)
{
 ...
 if(temp == RT)
  ...
  qts_write("at <b><font color=#357EC7>%g%cC</font></b>, gently aspirate out the supernatant and discard it.<br>", temp, 0x00B0);
...
void centrifuge_phases_top(Container &container1, Speed* speed1, r4 temp, Time* time1, Volume* aspirate_vol, Container &container2)
{
 ...
 if(temp == RT)
  ...
  qts_write(" at <b><font color=#357EC7>%g%cC</font></b>", temp, 0x00B0);
 fprintf( fp, " and aspirate out ");
 aspirate_vol->display_vol();
 qts_write(" of top layer into %s.<br>", container2.name);
...
void centrifuge(Container &container1, Speed* speed1, r4 temp, Time* time1)
{
 ...
 if(temp == RT)
  ...
  qts_write(" at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
 if(centrifuge_no == 1)
 {
  equipments[equip_no] = "Centrifuge";
  equip_no++;
...
{ 
 // graph maintenance
 {
  ...
  qts_write("Discard %s.<br>", container1.contents.new_name);
  container1.volume = 0;
...
{
 switch(type){
 case TAPPING: qts_write("Gently tap the mixture for ");time1->display_time();qts_write(" .<br>"); break;
 case STIRRING: qts_write("Stir the mixture for ");time1->display_time();qts_write(" .<br>"); break;
 case INVERTING: qts_write("Close the tube tightly and invert the tube ");time1->display_time();qts_write(" times.<br>"); break;
 case VORTEXING: qts_write("Vortex the mixture for ");time1->display_time();qts_write(" .<br>"); break;
...
void mix (Container &container1, enum mixing type, u4 times)     
{
 switch(type){
 case INVERTING: qts_write("Close the tube tightly and invert the tube <b><font color=#357EC7>%d times</font></b>.<br>", times); break;
 default: qts_write("Invalid entry.<br>");
 }
 // graph maintenance
 {
...
void mix (Container &container1, enum mixing type, u4 min_times, u4 max_times)     
{
 switch(type){
 case INVERTING: qts_write("Close the tube tightly and invert the tube <b><font color=#357EC7>%d - %d times</font></b>.<br>", min_times, max_times); break;
 default: qts_write("Invalid entry.<br>");
 }
 // graph maintenance
 {
...
{
 qts_write("Keep %s aside for ", container1.contents.new_name);
 time1->display_time();
 qts_write(".<br>");
 // graph maintenance
 {
...
void combine_and_mix (enum mixing type, Time* time1, u4 count, Container &container1, ...)
{
 va_list ap;
 va_start(ap, container1);
 combine_helper(count, container1, ap);
...
void combine_and_mix (enum mixing type, u4 count, Container &container1, ...)
{
 va_list ap;
 va_start(ap, container1);
 combine_helper(count, container1, ap);
...
void inoculation(Container& medium, Fluid& sample1, Volume* volume1, r4 temp, Time* time1, u4 shaking_type)
{
 char* unit;
 r4 result;
 if ((sample1.volume == 0)|| (sample1.volume < (volume1->value*volume1->mul)))
 {
  qts_write("<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", sample1.new_name);
 ...
 if (medium.volume != DEFAULT)
 {
  switch(medium.contents.unit){
   case UL: unit = "mcL"; result = medium.volume; break;
   case ML: unit = "ml"; result = medium.volume/1000; break;
   case L: unit = "l"; result = medium.volume/1000000; break;
   default:break;
  }
  qts_write("Inoculate <font color=#357EC7>%g %s %s</font> with ", result, unit, medium.contents.new_name);
 }
 else
  qts_write("Inoculate <font color=#357EC7>%s</font> with ", medium.contents.new_name);
 volume1->display_vol();
 switch(shaking_type)
 {
  case 1:
   qts_write(" of <font color=#357EC7>%s</font> and incubate with shaking for ", sample1.new_name);
   break;
  case 2:
   qts_write(" of <font color=#357EC7>%s</font> and incubate without shaking for ", sample1.new_name);
   break;
  default:qts_write("Invalid inputs.");break;
 }
 time1->display_time();
 qts_write(" at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
...
void inoculation(Container& medium, Fluid& sample1, Volume* volume1, r4 temp, Time* time1, r4 cell_density, u4 shaking_type)
{
 char* unit;
 r4 result;
 if ((sample1.volume == 0)|| (sample1.volume < (volume1->value*volume1->mul)))
 ...
 {
  switch(medium.contents.unit){
   case UL: unit = "mcL"; result = medium.contents.volume; break;
 ...
 time1->display_time();
 qts_write(" at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
 qts_write("Calibrate the spectrophotometer and make sure that the cell density of the culture has reached <b><font color=#357EC7>%g X 10<sup>6</sup> cells/ml</font></b> before continuing.<br>", cell_density);
 medium.volume = medium.volume + volume1->value * volume1->mul;
...
void inoculation(Container& medium, Solid& sample1, r4 temp, Time* time1, u4 shaking_type)
{
 char* unit;
 r4 result;
 if (usage_list_fluids[sample1.usage_index].original_name == sample1.original_name)
 ...
 {
  case 1:
   qts_write("<font color=#357EC7>%s</font> and incubate with shaking for ", sample1.new_name);
   time1->display_time();
   qts_write(" at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
   break;
  case 2:
   qts_write("<font color=#357EC7>%s</font> and incubate without shaking for ", sample1.new_name);
   time1->display_time();
   qts_write(" at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
   break;
  default:qts_write("Invalid inputs.");break;
...
void inoculation(Container& medium, Solid& sample1, r4 temp, Time* time1, r4 cell_density, u4 shaking_type)
{
 char* unit;
 r4 result;
 if (usage_list_fluids[sample1.usage_index].original_name == sample1.original_name)
...
void store_for(Container& container1, r4 temp, Time* time1)
{
 ...
 else 
 {
  qts_write("Store ");
  check_container(container1);
  qts_write(" at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
 }
 time1->display_time();
 qts_write(".<br>");
 // graph maintenance
...
void store_for(Container& sample1, r4 temp, Time* time1, enum func function)
{
 if(function == 1)
 {
  qts_write("Denature ");
...
void store_until(Container& container1, r4 temp, enum until type)
{
 // graph maintenance
 {
  ...
  switch(type)
  {
   case ETHANOL_EVAP:if (temp == RT)
          qts_write("Store %s at <b><font color=#357EC7>room temperature</font></b> until the ethanol has evaporated and no fluid is visible in the tube.<br>", container1.contents.new_name);
         else
          qts_write("Store %s at <b><font color=#357EC7>%g%cC</font></b> until the ethanol has evaporated and no fluid is visible in the tube.<br>", container1.contents.new_name, temp, 0x00B0);break;
   case OD:qts_write("Incubate %s at <b><font color=#357EC7>%g%cC</font></b> until the O.D.600 reaches 0.6.<br>", container1.contents.new_name, temp, 0x00B0);break;
   case THAW:qts_write("Allow %s to thaw at <b><font color=#357EC7>room temperature</font></b>.<br>", container1.contents.new_name);break;
   case COOLED:qts_write("Keep %s at <b><font color=#357EC7>room temperature</font></b> until cooled.<br>", container1.contents.new_name);break;
   case COLOUR_DEVELOPS:qts_write("Wait for the colour to develop.<br>");break;
   case THAW_ICE:qts_write("Allow %s to thaw on <b><font color=#357EC7>ice</font></b>.<br>", container1.contents.new_name);break;
   default:break;
  }
}

void store_until(Container& container1, r4 temp, enum until type, Time* time1)
{
 switch(type)
 {
  case ETHANOL_EVAP:if (temp == RT)
         qts_write("Store %s at <b><font color=#357EC7>room temperature</font></b> until the ethanol has evaporated and no fluid is visible in the tube (~", container1.contents.new_name);
        else 
         qts_write("Store %s at <b><font color=#357EC7>%g%cC</font></b> until the ethanol has evaporated and no fluid is visible in the tube (~", container1.contents.new_name);break;
  case OD:qts_write("Incubate %s at <b><font color=#357EC7>%g%cC</font></b> until the O.D.600 reaches 0.6 (~", container1.contents.new_name);break;
  case THAW:qts_write("Allow %s to thaw at <b><font color=#357EC7>room temperature</font></b> (~", container1.contents.new_name);break;
  case COOLED:qts_write("Keep %s at <b><font color=#357EC7>room temperature</font></b> until cooled (~", container1.contents.new_name);break;
  case COLOUR_DEVELOPS:qts_write("Wait for the colour to develop (~", container1.contents.new_name);break;
  default:break;
 }
 time1->display_time();
 qts_write(").<br>");
...
void invert_dry(Container& container1, r4 temp, Time* time1)
{
 if(temp == RT)
 {
  qts_write("Stand the tube containing %s for ", container1.contents.new_name);
  time1->display_time();
  qts_write(" in an inverted position on a paper towel to allow all of the fluid to drain away.<br>");
 }
 else 
 {
  qts_write("Stand the tube containing %s in an inverted position on a paper towel to allow all of the fluid to drain away. Dry at <b><font color=#357EC7>%g%cC</font></b> for ", container1.contents.new_name, temp, 0x00B0);
  time1->display_time();
  qts_write(".<br>");
 }
 container1.volume = 0;
...
void invert_dry(Container& container1, r4 temp)
{
 // graph maintenance
 {
  ...
  if(temp == RT)
   qts_write("Stand the tube containing %s in an inverted position on a tissue paper to allow all of the fluid to drain away.<br>", container1.contents.new_name);
  else 
   qts_write("Stand the tube containing %s in an inverted position on a tissue paper to allow all of the fluid to drain away.<br>", container1.contents.new_name);
  container1.volume = 0;
}

void incubate(Container& container1, r4 temp, Time* time1, u4 rpm)
{
 if(temp == RT)
 {
  qts_write("Incubate ");
 ...
 time1->display_time();
 qts_write(" with shaking at %d rpm.<br>", rpm);
 if(incubator_no == 1)
 {
  equipments[equip_no] = "Incubator";
  equip_no++;
...
void incubate(Container& container1, r4 temp, Time* time1)
{
 if(temp == 28)
 {
  qts_write("Incubate ");
...
void incubate_and_mix(Container& container1, r4 temp, Time* time1, Time* time_mix, enum mixing type)
{
 qts_write("Incubate ");
 check_container(container1);
 switch (type){
   case STIRRING: qts_write(" at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0); time1->display_time(); qts_write(", mixing gently by stirring the tube every "); time_mix->display_time(); qts_write(". <br>");break;
   case INVERTING: qts_write(" at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0); time1->display_time(); qts_write(", mixing gently by inverting the tube every "); time_mix->display_time(); qts_write(". <br>");break;
   case VORTEXING: qts_write(" at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0); time1->display_time(); qts_write(", mixing gently by vortexing the tube every "); time_mix->display_time(); qts_write(". <br>");break;
   default:break;
 }
 if(incubator_no == 1)
 {
...
void nanodrop(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write("Measure the concentration of %s in your final samples with the Nanodrop.<br>", container1.contents.new_name);
}
...
void time_constraint(Container& container1, Time* time1, enum time_constraint_type type)
{
 ...
 case CURRENT_FLUID: 
  if (time1 == 0)
    qts_write("<font color=red>NOTE: Use %s ", container1.contents.new_name);
  else
  {
   qts_write("<font color=red>NOTE: Use %s within ", container1.contents.new_name);
  }
  time1->display_time(); 
  qts_write(".</font><br>"); 
  break;
 case NEXTSTEP: 
  ...
  time1->display_time();qts_write("!</font><br>");break;
  default:
    break;
 }
 // graph maintenance
...
{
 qts_write("NOTE: If %s is/are used within ", container1.contents.new_name);
 time1->display_time();
 qts_write(", set temperature to <b><font color=#357EC7>%g%cC</font></b> and use, else store it at <b><font color=#357EC7>%g%cC</font></b>.", temp_use, 0x00B0, temp_store, 0x00B0);
...
void mixing_table(u4 no_rows, u4 no_columns, Volume* rxn_vol, Fluid array1[], char* tube[], r4 *vol, Container& container1)
{
 ...
 if(no_rows-1 == 1)
  qts_write("Use the following table as a checklist for preparing the reaction in %s:<br><br>", container1.name);
 else 
  qts_write("Use the following table as a checklist for preparing the reactions in %s:<br><br>", container1.name);
 ...
 for(i=0; i<no_rows; i++)
 {
  for(j=0; j<no_columns; j++)
  {
   ...
   {
    qts_write("<td><font color=#357EC7>%s</font></td>", (array1[j-1]).new_name);
    if (j == no_columns-1)
      qts_write("</tr></thead>");
   }
   else if((i != 0) && (j == 0))
     qts_write("<tbody><tr><td><font color=#357EC7>%s</font></td>", tube[i-1]);
   else 
   {
    if((i == no_rows-1 )&& (j == no_columns-1) && ((*(vol+(no_columns-1)*(i-1)+(j-1)) == XVAL)))
    {
     ...
     qts_write("</font></b></td>");
      ...
      qts_write("<td><b><font color=#357EC7>%g</font></b></td>", *(vol+(no_columns-1)*(i-1)+(j-1)));
    if (j == no_columns-1)
      qts_write("</tr></body>");
   }
  }
 ...
 for(i=1; i<no_rows; i++)
 {
  for(j=1; j<no_columns; j++)
  {
   ...
   {
    qts_write("<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", array1[j-1].new_name);
...
void mixing_table(u4 no_rows,u4 no_columns, Fluid array1[], char* tube[], Volume* volumes[], Volume* rxn_vol, Container& container1)
{
 ...
 for (i=0; i<no_columns-1;i++)
 {
  ...
  else
  {
   ...
   {
    qts_write("<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", array1[i].new_name);
   }
...
void plate_out(Plate& plate1, Container& container1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  {
   qts_write("<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", container1.contents.new_name);
  ...
  qts_write("Plate out ");
  volume1->display_vol();
  qts_write(" of %s onto ", container1.contents.new_name);
  if(plate1.state == "")
   qts_write("<font color=#357EC7>%s</font>.<br>", plate1.new_name);
  else
   qts_write("<a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", plate1.new_name, plate1.new_name);
  container1.volume = container1.volume - volume1->value * volume1->mul;
}
...
void plate_out(Plate& plate1, Container& container1)
{
 // graph maintenance
 {
  ...
  if(plate1.state == "") 
   qts_write("Plate out %s onto <font color=#357EC7>%s</font>.<br>", container1.contents.new_name, plate1.new_name);
  else
   qts_write("Plate out %s onto <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", container1.contents.new_name, plate1.new_name, plate1.new_name);
  container1.volume = 0;
}

void name_container(Container& container1, char* name)
{
 qts_write("Set aside a fresh %s. Call it %s. <br>", container1.name, name);
...
void thermocycler (Container& master_mix, u4 cycles, r4 temp1, Time* time1, r4 temp2, Time* time2, r4 temp3, Time* time3, enum pcr_type type)
{
 u4 ttr1 = total_time_required;
 u4 ttr2;
 if(type == NORMAL)
 {
  qts_write("Thermocycling<br><ul><li>No. of cycles: <b><font color=#357EC7>%d</font></b></li>", cycles);
  qts_write("<li>Denature: <b><font color=#357EC7>%g%cC</font></b>, ", temp1, 0x00B0);
  time1->display_time();
  qts_write("</li> <li> Anneal: <b><font color=#357EC7>%g%cC</font></b>, ", temp2, 0x00B0);
  time2->display_time();
  qts_write("</li> <li>Elongate: <b><font color=#357EC7>%g%cC</font></b>, ", temp3, 0x00B0);
  time3->display_time();
  qts_write("</li></ul>");
  ...
  qts_write("Carry out colony PCR of %s.<br>", master_mix.contents.new_name);
...
void thermocycler (Container& master_mix, u4 cycles, r4 temp1, Time* time1, r4 temp2, Time* time2, r4 temp3, Time* time3, enum pcr_type type, r4 grad_temp)
{
 u4 ttr1 = total_time_required;
 u4 ttr2;
 if(type == NORMAL)
 ...
 else if(type == GRADIENT)
 {
  ...
  qts_write("<li>Denature: <b><font color=#357EC7>%g%cC</font></b>, ", temp1, 0x00B0);
  time1->display_time();
  qts_write("</li> <li> Anneal: <b><font color=#357EC7>%g%cC- %g%cC (gradient PCR)</font></b>, ", temp2, 0x00B0, grad_temp, 0x00B0);
  time2->display_time();
  qts_write("</li> <li>Elongate: <b><font color=#357EC7>%g%cC</font></b>, ", temp3, 0x00B0);
  time3->display_time();
  qts_write("</li></ul>");
...
void thermocycler (Container& master_mix, r4 temp1, Time* time1, r4 temp2, Time* time2)
{
 ...
 qts_write("<ul><li><b><font color=#357EC7>%g%cC</font></b>, ", temp1, 0x00B0);
 time1->display_time();
 qts_write("</li> <li><b><font color=#357EC7>%g%cC</font></b>, ", temp2, 0x00B0);
 time2->display_time();
 qts_write("</li></ul>");
 if(thermocycler_no == 1)
 {
...
void thermocycler (Plate& plate1, enum pcr_type type)
{
 // graph maintenance
 {
  ...
  if(type == COLONY)
   qts_write("Carry out colony PCR on %s.<br>", plate1.new_name);
...
void ce_detect (Container& container1, r4 length, r4 volt_per_cm, Fluid& fluid1)
{
 // graph maintenance
 {
  ...
  qts_write("Detect/separate %s by capillary electrophoresis with the following settings - <b><font color=#357EC7>%g</font></b> cm at <b><font color=#357EC7>%g</font></b> V/cm using %s.<br>", container1.contents.new_name, length, volt_per_cm, fluid1);
...
void ce_detect (Container& container1, r4 length, r4 volt_per_cm, Fluid& fluid1, Time* time1)
{
 ...
 qts_write("Detect/separate %s by capillary electrophoresis with the following settings - <b><font color=#357EC7>%g</font></b> cm at <b><font color=#357EC7>%g</font></b> V/cm using %s for ", container1.contents.new_name, length, volt_per_cm, fluid1.new_name);
 time1->display_time();
 qts_write(".<br>");
 name_sample(container1, "separated flow");
 // graph maintenance
...
void measure_fluorescence (Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write("Measure the fluorescence of %s.<br>", container1.contents.new_name);
...
void centrifuge_column(Column& column1, Speed* speed1, r4 temp, Time* time1)
{
 ...
 if(temp==RT);
  ...
  qts_write("at <b><font color=#357EC7>%g%cC</font></b>s", temp, 0x00B0);
 qts_write(".<br>");
 if(centrifuge_no == 1)
 {
  equipments[equip_no] = "Centrifuge";
...
void add_to_column(Column& column1, Fluid& fluid1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  if ((fluid1.volume == 0)|| (fluid1.volume < (volume1->value*volume1->mul)))
  {
   qts_write("<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", fluid1.new_name);
  ...
  volume1->display_vol();
  if(fluid1.state == "")
   qts_write(" of <font color=#357EC7>%s</font> to %s.<br>", fluid1.new_name, column1.name);
  else 
   qts_write(" of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", fluid1.new_name, fluid1.new_name, column1.name);
...
void add_to_column(Column& column1, Container& container1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  if(container1.contents.state == "")
   qts_write(" of <font color=#357EC7>%s</font> %s.<br>", container1.contents.new_name);
  else
   qts_write(" of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> %s.<br>", container1.contents.new_name, container1.contents.new_name);
...
void add_to_column(Column& column1, Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write("Add <font color=#357EC7>%s</font> to %s.<br>", container1.contents.new_name, column1.name);
  column1.contents = container1.contents;
...
void transfer_column(Column& column1, Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write("Transfer %s into %s.<br>", column1.name, container1.name);
}
...
void collect_tissue(Tissue& sample1, r4 wt, enum weight_unit unit, Container& container1)
{
 // graph maintenance
 {
  ...
  switch(unit)
  {
   case UG: wu="\B5g";mul = MICRO;break;
...
void homogenize_tissue(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write("Add %s into the mortar containing liquid nitrogen and grind with a pestle until it is a fine powder.<br>", container1.contents.new_name);
  comment("Use gloves!");
  if(mortar_no==1)
  {
...
void homogenize_tissue(Container& container1, Fluid& fluid1)
{
 // graph maintenance
 {
  ...
  qts_write("Homogenize %s in %s.<br>", container1.contents.new_name, fluid1.new_name);
  name_sample(container1, "homogenized tissue");
...
void electrophoresis(u4 no_samples, Container container_array[], r4 agar_conc, Volume* pdt_vol, Volume* loading_buff_vol)
{
 ...
 // graph maintenance
 {
  ...
  if(no_samples == 2)
   qts_write("%s and %s (", container_array[0].contents.new_name, container_array[1].contents.new_name);
  else
  {
   for(i=0; i<no_samples;i++)
   {
    if(i=no_samples-1)
     qts_write("%s (", container_array[i].contents.new_name);        
    else if(i=no_samples-2)
     qts_write("%s and ", container_array[i].contents.new_name);
    else
     qts_write("%s, ", container_array[i].contents.new_name);
...
void pcr_init_denat(Container& container1, r4 temp, Time* time1)
{
 ...
 qts_write("Initial denaturation<br><ul><li>Denature: <b><font color=#357EC7>%g%cC</font></b>, ", temp, 0x00B0);
 time1->display_time();
 qts_write("</li></ul>");
 // graph maintenance
 {
...
void pcr_final_ext(Container& sample1, r4 temp_ext, Time* time_ext, r4 temp_halt, Time* time_halt, r4 temp_hold)
{
 ...
 qts_write("</li><li>Halt reaction: <b><font color=#357EC7>%g%cC</font></b>, ", temp_halt, 0x00B0);
 time_halt->display_time();
 qts_write("</li><li>Hold: <b><font color=#357EC7>%g%cC</font></b>, until removed from machine </li></ul>", temp_hold, 0x00B0);
 // graph maintenance
 {
...
void incubate_plate(Plate& plate1, r4 temp, Time* time1)
{
 ...
 if(temp == RT)
  qts_write("Incubate %s at <b><font color=#357EC7><b><font color=#357EC7>room temperature</font></b></font></b> for ", plate1.new_name);
 else if((temp == ON_ICE)||(temp == 0))
  qts_write("Incubate %s on <b><font color=#357EC7><b><font color=#357EC7>ice</font></b></font></b> for ", plate1.new_name);
 else
  qts_write("Incubate %s at <b><font color=#357EC7>%g%cC</font></b> for ", plate1.new_name, temp, 0x00B0);
 time1->display_time();
 qts_write(".<br>");
 if(incubator_no == 1)
 {
...
void sequencing(Container& container1)
{
 // graph maintenance
 {
  ...
  fprintf(fp,"Dilute %s to <font color=#357EC7>100ng/ mcL</font> and send <font color=#357EC7>1 \B5g (10 mcL)</font> for sequencing.<br>", container1.contents.new_name);
}
...
void weigh(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write("Weigh the amount of %s present.<br>", container1.contents.new_name);
}
...
void immerse_slide(Slide& slide1,Fluid& sample1, Time* time1, enum func function, u4 no_of_times)
{
 char* noft;
 u4 ttr1 = total_time_required;
 u4 ttr2;
 ...
 if (sample1.state == "")
 {
  switch(function)
  {
   case DEWAX: qts_write("Dewax %s in <b><font color=#357EC7>%s</font></b> for ",slide1.name, sample1.new_name);break;
   default:break;
  }
 }
 else{
  switch(function)
  {
   case DEWAX: qts_write("Dewax %s in <a href=\"#%s\" >%s</a> for ", slide1.name, sample1.new_name, sample1.new_name);break;
   default:break;
  }
 }
 time1->display_time();
...
void immerse_slide(Slide& slide1, Fluid& sample1, Time* time1, u4 no_of_times)
{
 ...
 if (sample1.state == "")
  qts_write("Immerse %s in <b><font color=#357EC7>%s</font></b> for ", slide1.name, sample1.new_name);
 else 
  qts_write("Immerse %s in <a href=\"#%s\" >%s</a> for ", slide1.name, sample1.new_name, sample1.new_name);
 time1->display_time();
 qts_write(" %s.<br>", noft);
 ttr2 = total_time_required-ttr1;
 total_time_required = total_time_required + ttr2 * no_of_times;
...
void immerse_slide(Slide& slide1,Fluid& sample1, u4 no_of_times)
{
 // graph maintenance
 {
  ...
  if ((sample1.state == "")&&(no_of_times == 1))
   qts_write("Immerse %s in %s.<br>", slide1.name, sample1.new_name);
  else if ((sample1.state == "")&&(no_of_times > 1))
   qts_write("Immerse %s in %s %s.<br>", slide1.name, sample1.new_name, noft);
  else if (no_of_times > 1)
   qts_write("Immerse %s in <a href=\"#%s\" >%s</a> %s.<br>", slide1.name, sample1.new_name, sample1.new_name, noft);
  else 
   qts_write("Immerse %s in <a href=\"#%s\" >%s</a>.<br>", slide1.name, sample1.new_name, sample1.new_name);
  ttr2 = total_time_required-ttr1;
  total_time_required = total_time_required + ttr2 * no_of_times;
}
...
void remove_slide(Slide& slide1, Fluid& sample1)
{
 // graph maintenance
 {
  ...
  qts_write("Take out %s from %s.<br>", slide1.name, sample1.new_name);
}

void wash_tissue(Tissue& tissue1, Fluid& sample1, enum wash_type type, r4 temp, u4 shaking)
{
 // graph maintenance
 {
  ...
  if(sample1.state == "")
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <font color=#357EC7>%s</font>", tissue1.new_name, sample1.new_name);break;
    case RINSING:qts_write("Rinse %s in <font color=#357EC7>%s</font>", tissue1.new_name, sample1.new_name);break;
    default:qts_write("Wash %s in <font color=#357EC7>%s</font>", tissue1.new_name, sample1.new_name);break;
   }
  }
  else
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <a href=\"#%s\" >%s</a>", tissue1.new_name, sample1.new_name, sample1.new_name);break;
    case RINSING:qts_write("Rinse %s in <a href=\"#%s\" >%s</a>", tissue1.new_name, sample1.new_name, sample1.new_name);break;
    default:qts_write("   Wash %s in <a href=\"#%s\" >%s</a>", tissue1.new_name, sample1.new_name, sample1.new_name);break;
   }
  }
  if (temp == RT)
   qts_write(" at <b><font color=#357EC7>room temperature</font></b>%s.<br>", shake_type);
  else
   qts_write(" at <b><font color=#357EC7>%g%cC</font></b>%s.<br>", temp, 0x00B0, shake_type);
  if(shaking == 1)
  {
   if(shaker_no == 1)
   {
...
void wash_tissue(Tissue& tissue1, Fluid& sample1, enum wash_type type, Time* time1, u4 no_of_times, r4 temp, u4 shaking)
{
 ...
 if(sample1.state == "")
 {
  if (no_of_times == 1)
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <font color=#357EC7>%s</font> for ", tissue1.new_name, sample1.new_name);break;
    case RINSING:qts_write("Rinse %s in <font color=#357EC7>%s</font> for ", tissue1.new_name, sample1.new_name);break;
    default:break;
   }
   time1->display_time();
   if (temp == RT)
  ...
  else
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <font color=#357EC7>%s</font>, <b><font color=#357EC7>%s</font></b> for ", tissue1.new_name, sample1.new_name, noft);break;
    case RINSING:qts_write("Rinse %s in <font color=#357EC7>%s</font>, <b><font color=#357EC7>%s</font></b> for ", tissue1.new_name, sample1.new_name, noft);break;
    default:break;
   }
   time1->display_time();
   if (temp == RT)
    qts_write(" each at <b><font color=#357EC7>room temperature</font></b>%s.<br>", shake_type);
   else
    qts_write(" each at <b><font color=#357EC7>%g%cC</font></b>%s.<br>", temp, 0x00B0, shake_type);
  }
 }
 else 
 {
  if (no_of_times == 1)
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <a href=\"#%s\" >%s</a> for ", tissue1.new_name, sample1.new_name, sample1.new_name);break;
    case RINSING:qts_write("Rinse %s in <a href=\"#%s\" >%s</a> for ", tissue1.new_name, sample1.new_name, sample1.new_name);break;
    default:break;
   }
   time1->display_time();
   if (temp == RT)
  ...
  else
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <a href=\"#%s\" >%s</a>, <b><font color=#357EC7>%s</font></b> for ",tissue1.new_name, sample1.new_name, sample1.new_name, noft);break;
    case RINSING:qts_write("Rinse %s in <a href=\"#%s\" >%s</a>, <b><font color=#357EC7>%s</font></b> for ",tissue1.new_name, sample1.new_name, sample1.new_name, noft);break;
    default:break;
   }
   time1->display_time();
   if (temp == RT)
...
void immerse_slide_and_incubate(Slide& slide1, Fluid& sample1, r4 temp, Time* time1)
{
 ...
 qts_write("Incubate %s for ", slide1.name);
 time1->display_time();
 if(sample1.state == "")
 {
  if(temp==RT)
   qts_write(" in <b><font color=#357EC7>%s</font></b> at <b><font color=#357EC7>room temperature</font></b>.<br>", sample1.new_name);
  else
   qts_write(" in <b><font color=#357EC7>%s</font></b> at <b><font color=#357EC7>%g%cC</font></b>.<br>", sample1.new_name, temp, 0x00B0);
 }
 else
 {
  if(temp==RT)
   qts_write(" in <a href=\"#%s\" >%s</a> at <b><font color=#357EC7>room temperature</font></b>.<br>", sample1.new_name, sample1.new_name);
  else
   qts_write(" in <a href=\"#%s\" >%s</a> at <b><font color=#357EC7>%g%cC</font></b>.<br>", sample1.new_name, sample1.new_name, temp, 0x00B0);
 }
 // graph maintenance
 {
...
void immerse_slide_and_incubate(Slide& slide1, Fluid& sample1, r4 temp, Time* time1, enum container_type type)
{
 if (usage_list_fluids[sample1.usage_index].original_name == sample1.original_name)
 usage_list_fluids[sample1.usage_index].used = 1;
 else 
  ...
  case 5:
   ...
   if(sample1.new_name == "")
   {
    if(type == HUMIDIFIED_CHAMBER)
     qts_write(" in <b><font color=#357EC7>%s</font></b> at <b><font color=#357EC7>room temperature</font></b> in humidified chamber.<br>", sample1.new_name);
   }
   else
   {
    if(type == HUMIDIFIED_CHAMBER)
     qts_write(" in <a href=\"#%s\" >%s</a> at <b><font color=#357EC7>room temperature</font></b> in humidified chamber.<br>", sample1.new_name, sample1.new_name);
   }
   break;
  default:break;
 }
...
void add_to_slide(Slide& slide1, Fluid& sample1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  if(sample1.state == "")
  {
   qts_write("Add ");
   volume1->display_vol();
   qts_write(" of <b><font color=#357EC7>%s</font></b> to each of the %s.<br>", sample1.new_name, slide1.name);
  }
  else
  {
   qts_write("Add ");
   volume1->display_vol();
   qts_write(" of <a href=\"#%s\" >%s</a> to each of the %s.<br>", sample1.new_name, sample1.new_name, slide1.name);
  }
}
...
void dry_slide(Slide& slide1)
{
 // graph maintenance
 {
  ...
  qts_write("Dry %s.<br>", slide1.name);
}
...
void mount_observe_slide(Slide& slide1, Fluid& fluid1, char* device)
{
 // graph maintenance
 {
  ...
  qts_write("Mount %s with <b><font color=#357EC7>%s</font></b>, let it solidify and view under %s.<br>", slide1.name, fluid1.new_name, device);
}
...
void facs(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write("FACS: sort %s based on fluorescence.", container1.contents.new_name);
}
...
void electroporate (Container& container1, r4 voltage, u4 no_pulses)
{
 // graph maintenance
 {
  ...
  qts_write("Set the electroporator to deliver <b><font color=#357EC7>%g V</font></b>, and then press the PULSE button <b><font color=#357EC7>%d</font></b> times. <br>", voltage, no_pulses);
  if(electro_no==1)
  {
...
void store_plate(Plate& plate1, r4 temp)
{
 // graph maintenance
 {
  ...
  qts_write("Store %s at <b><font color=#357EC7>%g%cC</font></b>.<br>", plate1.new_name, temp, 0x00B0);
}
...
void wash_slide(Slide& slide1, Fluid& sample1, enum wash_type type)
{
 ...
 if(sample1.state == "")
 {
  switch(type)
  {
   case WASHING:qts_write("Wash %s in <b><font color=#357EC7>%s</font></b>.<br>", slide1.name, sample1.new_name);break;
   case RINSING:qts_write("Rinse %s in <b><font color=#357EC7>%s</font></b>.<br>", slide1.name, sample1.new_name);break;
   default:qts_write("Wash %s in <b><font color=#357EC7>%s</font></b>.<br>", slide1.name, sample1.new_name);break;
  }
 }
 else
 {
  switch(type)
  {
   case WASHING:qts_write("Wash %s in <a href=\"#%s\" >%s</a>.<br>", slide1.name, sample1.new_name, sample1.new_name);break;
   case RINSING:qts_write("Rinse %s in <a href=\"#%s\" >%s</a>.<br>", slide1.name, sample1.new_name,  sample1.new_name);break;
   default:qts_write("   Wash %s in <a href=\"#%s\" >%s</a>.<br>", slide1.name, sample1.new_name, sample1.new_name);break;
  }
 }
}

void wash_slide(Slide& slide1, Fluid& sample1, enum wash_type type, Time* time1, u4 no_of_times)
{
 char *noft;
 u4 ttr1 = total_time_required;
 u4 ttr2;
 ...
 if(sample1.state == "")
 {
  if (no_of_times == 1)
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <font color=#357EC7>%s</font> for ", slide1.name, sample1.new_name);break;
    case RINSING:qts_write("Rinse %s in <font color=#357EC7>%s</font> for ", slide1.name, sample1.new_name);break;
    default:break;
   }
   time1->display_time();
   qts_write(".<br>");
  ...
  else
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <font color=#357EC7>%s</font>, <b><font color=#357EC7>%s</font></b> for ", slide1.name, sample1.new_name, noft);break;
    case RINSING:qts_write("Rinse %s in <font color=#357EC7>%s</font>, <b><font color=#357EC7>%s</font></b> for ", slide1.name, sample1.new_name, noft);break;
    default:break;
   }
   time1->display_time();
   qts_write(" each.<br>");
 ...
 else 
 {
  if (no_of_times == 1)
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <a href=\"#%s\" >%s</a> for ", slide1.name, sample1.new_name, sample1.new_name);break;
    case RINSING:qts_write("Rinse %s in <a href=\"#%s\" >%s</a> for ", slide1.name, sample1.new_name, sample1.new_name);break;
    default:break;
   }
   time1->display_time();
   qts_write(".<br>");
  ...
  else
  {
   switch(type)
   {
    case WASHING:qts_write("Wash %s in <a href=\"#%s\" >%s</a>, <font color=#357EC7>%s</font> for ", slide1.name, sample1.new_name, sample1.new_name, noft);break;
    case RINSING:qts_write("Rinse %s in <a href=\"#%s\" >%s</a>, <font color=#357EC7>%s</font> for ", slide1.name, sample1.new_name, sample1.new_name, noft);break;
    default:break;
   }
   time1->display_time();
   qts_write(" each.<br>");
...
void measure_solid(Solid& solid1, r4 weight, enum weight_unit unit, Container& container1)
{
 // graph maintenance
 {
  ...
  switch(unit){
 case UG: type = "\B5g";mul = MICRO;break;
 ...
 default : type = "mg";mul = MICRO;break;
  ...
  {
   if (solid1.state == "")
    qts_write("Measure out <b><font color=#357EC7>%g %s</font></b> of <font color=#357EC7>%s</font> into %s.<br>", weight, type, solid1.new_name, container1.name);
   else
    qts_write("Measure out <b><font color=#357EC7>%g %s</font></b> of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", weight, type, solid1.new_name, solid1.new_name, container1.name);
   prev_container = container1.name;
   prev_cont++;
  }
  else if( prev_container == container1.name)
  {
   if (solid1.state == "")
    qts_write("Add <b><font color=#357EC7> %g %s</font></b> of <font color=#357EC7>%s</font>.<br>", weight, type, solid1.new_name);
   else
    qts_write("Add <b><font color=#357EC7> %g %s</font></b> of <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", weight, type, solid1.new_name, solid1.new_name);
  }
...
void measure_solid(Solid& solid1, Container& container1)
{
 // graph maintenance
 {
  ...
  {
   if (solid1.state == "")
    qts_write("Measure out <font color=#357EC7>%s</font> into %s.<br>", solid1.new_name, container1.name);
   else
    qts_write("Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", solid1.new_name, solid1.new_name, container1.name);
   prev_container = container1.name;
   prev_cont++;
  }
  else if( prev_container == container1.name)
  {
   if (solid1.state == "")
    qts_write("Add <font color=#357EC7>%s</font>.<br>", solid1.new_name);
   else
    qts_write("Add <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", solid1.new_name, solid1.new_name);
  }
  else if (solid1.state == "")
  {
   qts_write("Measure out <font color=#357EC7>%s</font> into %s.<br>", solid1.new_name, container1.name);
...
Symbol new_symbol(char* symbol, char* def)
{
 ...
 qts_write("<li>%s - %s</li>", symbol, def);
 return s1;
}
...
void set_value(Symbol& symbol1, r4 value, enum vol_unit unit)
{
 symbol1.value = value;
 symbol1.unit = unit;
}

void set_value(Symbol_t& symbol1, r4 value, enum time_unit unit)
{
 symbol1.value = value;
 symbol1.unit = unit;
}
...
void streak_plate(Plate& plate1, Plate& colony)
{
 // graph maintenance
 {
  ...
  qts_write("Use a sterile toothpick or disposable loop to transfer a small segement of %s to a streak or patch on %s. <br>", colony.new_name, plate1.new_name);
}
