...
--- not a cloud service --- x nevertheless incoporates 
...
\API{}s, etc.  Another feature of x is that it 
...
using.  In short, x can be seen as akin to 
...
--- or as x uses the term, \q{interactive/digital 
...
interaction.  The x approach recognizes 
...
programming environment.  The x approach 
...
The main difference between x and SeCo (apart 
from \Cpp{} instead of \Java{} being 
the underlying programming language) is 
that x cells are not intended, in the general 
case, to be typed in by programmers directly.  
Instead, x cells are normally constructed 
...
loaded into x need to provide a 
...
\p{The typical x notebook combines, at a 
...
Loading a data set into a x notebook 
