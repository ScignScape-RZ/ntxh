...
\geometry{left=.83in,top=.4in,right=.81in,bottom=1.1in} %margins
...
{\setlength{\fboxsep}{32pt}
	\relscale{1.2}{{\fontfamily{qcr}\fontseries{b}\selectfont%
{The \llMOSAIC{} Publication Repository and 
Structured Research Reporting Framework}
}}}
\end{center}
\end{minipage}
\end{tcolorbox}
...
\section{Overview}
\p{In recent years --- as publishing has become more \q{digitized} --- there
have emerged many online platforms for scientists to share and publish
...
can locate research based on author's name, keywords, or subject
matter, and a document viewer where readers can view each
publication's abstract (and sometimes its full text) and other
publication details (such as co-authors, dates, citations, etc.).
...
\p{The \q{\MOSAIC{}} system is a suite of code libraries allowing 
institutions to host publication repositories which address the 
limitations of existing scientific document portals 
...
linked to a \q{Supplemental Archive} which contains information about
the author's research methods, data sets, and focal topics.  Using 
\MOSAIC{}, developers can implement a hosting platform
for this supplemental material, in addition to publications themselves, 
where the platform provide software
...
\p{\lMOSAIC{} repositories can be general-purpose portals 
hosting academic or technical papers covering a broad range 
of topics; or narrower collections of articles focused 
on specific subject areas or institutional archives.  
Organizations can use use \MOSAIC{} internally as the 
basis of a document management system (\DMS{}).  Conversely, 
\MOSAIC{} could be used to provide open public access to 
...
\MOSAIC{} license).  \lMOSAIC{} can also be customized 
...
publications and Supplemental Archives can be embedded in 
scientific-computing applications, via \q{\MOSAIC{} plugins.}  
This ensures that publications and data sets can be 
examined interactively with the same software as scientists 
use to conduct research.  \lMOSAIC{} also introduces a 
\q{structured reporting system} (\MOSAICSR{}) for describing 
research/experimental/lab methods and protocols.  Supplemental Archives, 
plugins, and \MOSAICSR{} are outlined in the following sections.}
...
\item Representations of analytic methods and algorithms used to derive
the research findings, provided directly via computer code or
indirectly via formal descriptions of computational workflows;

\item Self-contained computer software demonstrating code which the
author developed and/or used for analyzing research data;

\item Multi-media assets such as audio or video files, annotated images,
\ThreeD{} graphics, interactive \TwoD/\ThreeD{} plots and diagrams, or other kinds of
non-textual content which needs to be viewed with special multi-media
software.
\end{enumerate}
}
...
used to obtain raw data.  In the former case, a typical 
archive will include a \i{data-set application}, or a semi-autonomous 
software component allowing researchers to study and visualize 
...
implementations of a \q{data-set explorer} (\MdsX{}) which 
can be adopted to the specific kind of information needing 
to be serialized for a given publication.  The resulting 
data-set application can then be packaged as a plugin to 
existing scientific software, depending on the relevant 
scientific discipline to which the research belongs.}
...
biology and bioimaging could be read within \CaPTk{}, an image-processing
...
and reused within the computing environments which scientists actually use to 
conduct professional research.  This is different 
than existing science-publication portals, which generally rely on web browsers to
access supplemental materials like data sets and multimedia files ---
in this setup, as a result, the software ecosystem wherein readers examine
published research is fundamentally separate from the software
platforms where research is actually conducted.  This is one example
of how existing portals are limited in their ability to share research
in rigorously reusable and replicable ways, and how \MOSAIC{} 
offers a potential improvement.}

\p{For authors, embedding presentations of their research within existing
scientific software has the added benefit of making their work 
more practically and immediately useful for the scientific community.  
Such presentations establish the computational framework for practically 
...
software such as \CaPTk{}, so that readers could actually run the
author's code as a \CaPTk{} module.  This kind of
functional assessment and adoption of novel contributions is harder to carry
out if a research work is only described indirectly within article
text, as opposed to being concretely developed for execution within a
...
protein or a chemical compound, say, can be viewed online via WebGL;
such graphics could even be embedded as an \q{iframe} within an HTML
...
multimedia, so the scientific software is a better forum for
showcasing novel research.}

\section{\protect\llMOSAIC{} Structured Reporting}
...
to use Supplemental Archives to document how thier research 
...
\bhref{https://jbioleng.biomedcentral.com/articles/10.1186/1754-1611-4-13}.} (which 
is included, in an updated \Cpp{} version, as one 
...
the \Pandore{} project provides an \q{Image Processing 
...
the \Pandore{} controlled vocabulary.}
...
in scientific software.  For these latter cases, 
...
are also cases where both scenarios are 
...
\p{Included in any \MOSAIC{} plugin would be specially-designed \PDF{} viewers for
interactively reading student's papers.  In particular, these \PDF{}
applications would recognize cross-references linking between student
publications and their associated Supplemental Archives.  This would
allow readers to identify concepts which are discussed and/or
represented both in the research paper and in the archive, annotating
their papers accordingly.  For example (a similar example is used in
the documentation for \BioCoder{}), 
the concept \q{\RNA{} Extraction} may be discussed in a
publication text, and also formally declared as one step in the lab
processing as represented via \BioCoder{}, and summarized in a
...
experimental process, so that readers could view the \BioCoder{} summary
as a context-menu action associated with the phrase where it appears
in the \PDF{} file.  For a different example, the phrase \q{Oxygenated
Airflow} refers to airflow in assisted-breathing devices, such as
ventilators; to ensure that the device is working properly, the
equipment must be monitored to ensure that a steady stream of oxygen
...
publication location of the \q{Oxygenated Airflow} phrase should once
...
software).  The \Pandore{} \q{Image Processing Objectives} 
...
theory).  In \MOSAICSR{}, the analogous concept is 
...
cytometric experiments which investigate cells and 
...
than \XML{}-based context (along with an expanded 
...
schema with Object-Oriented code.  In 
short, \MOSAICSD{} 
extends \DSPIN{} by defining  
...
we can assume that the workstation linking against a modified 
...
assume that the host application which which loads 
an enhanced, \q{Semantic} version of a \DICOM{} client library 
has the ability to consume more complex clinical data 
...
with a secondary application tailored to the extensions 
in particular.}
...
could be bundled into \DICOM{} files even if \DICOM{} clients 
...
a Covid-specific application could be accessed by 
...
bioimaging context) could then be extended only as much 
...
\p{Apart from metadata and keyword searches, an additional 
layer of search functionality within \MOSAIC{} portals 
is the option to search across Supplemental Archive 
contents such as data sets and research protocol descriptions.  
\lMOSAIC{} provides a Hypergraph-based database engine that 
can be used, at a minimum, for publication info; developers 
have the option of including supplemental content as well 
...
necessarily shared with the host \MOSAIC{} repository except 
by directly examining archive files.  However, a repository 
could choose to model some or all archive content within 
its publication database, either directly storing 
archives' data sets or importing certain select information 
(such as object/type schema or representative data samples).  
In this case, some data in the Supplemental Archives will be 
mirrored in the central database, so that it is accessible 
for user-facing searches.  \lMOSAIC{} employs a 
flexible Hypergraph Database engine which can accommodate 
many different kinds of data models, from \q{\SQL{}-like} 
tables to graphs and hierarchical collections types.  
Therefore, the publication database within a \MOSAIC{} 
portal would have the flexibility to import most data 
which is present in supplemental material associated 
with given publications.} 

\p{In some contexts, authors may choose specifically 
to structure their shared research data in a format that 
can be exported to a \MOSAIC{} publication database.  
This is because \MOSAIC{}'s hypergraph data model 
...
which is sufficiently rigorous to 
serve as a proxy for formal interface 
definitions.  
\lXML{} formats are a good case-study 
...
relevant data is just to explain how an \XML{} 
...
is usually implicit and unstated, but in \MOSAIC{}, such 
...
software which reads or creates these documents.}
...
two levels: on the one hand, they constrain 
the organization of \GTagML{} files themselves; 
but they also stipulate how \GTagML{} document 
structure relates to the type systems of 
code libraries serializing and deserializing 
\GTagML{} files.  To model this second, type-oriented 
...
programmers may then compose \q{infoset classes} 
...
with \ConceptsDB{}, which is the new hypergraph database 
