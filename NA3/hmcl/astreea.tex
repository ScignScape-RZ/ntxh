...
a line of dispenser products in which are embedded sensors which 
...
deploy fill-level monitors (for both solids and liquids) as part 
...
general, the sensor providers also maintain a central 
web service, such as \CCN{}, which integrates data from all 
buildings/locations into one data access point.  The software 
stack directly managed by Astreea, then, would need to interface 
...
sensor data can be installed directly on computers in 
...
obtain data, nor could then browse the web in general 
while using the custom application.  In addition to 
this basic convenience, customized desktop applications 
...
data visualization.  Moreover, by acquiring raw 
sensor data, companies can perform their own market 
analytics customer metrics, rather than relying exclusively 
on the analytic information shared by sensor manufacturers.  
...
versions, *do* support \ThreeD{} graphics to a limited extent.  These require
