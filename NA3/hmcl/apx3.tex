...
a stream of cells is 
investigated by passing them through 
laser beams where the cells' material 
absorbs and/or scatters the light waves, 
yielding patterns of light intensity 
along two axes --- one parallel to the 
laser source, and one derived from sensors at right angles to 
the source, engineered to detected  light scattering.  
...
in this process are not literal photographs 
...
--- selecting two-dimensional regions of interest 
...
The most comprehensive open-source package for working with 
the \FCS{} format is based on \R{}, although 
one \Cpp{} library (developed by the 
Fred Hutchinson Cancer Research Center) has been isolated from 
...
or \bhref{https://mbio.asm.org/content/10/5/e00317-19}.}
...
\lCRtwo{} will also introduce an 
object-oriented data model 
which consolidates various Flow Cytometry metadata 
standards (\MIFlowCyt{}, \GatingML{}, the Classification 
Results File Format, \flowCL{} 
for cell labeling, etc.) into a single library that can be embedded 
...
has been employed to support vaccination logistics 
in several ways.  The most straightforward 
is to map sites or geographic areas where vaccines have been 
...
--- and then superimposes on the map-image 
...
information is converted to markings or text 
...
in the previous subsection, providing code 
for integrating immunological and image-processing 
