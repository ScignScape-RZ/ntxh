...
\titlespacing*{\section}
{0pt}{3.8ex plus 1ex minus .1ex}{-0.2ex plus .2ex}

\titlespacing*{\subsection}
{0pt}{2.8ex plus 1ex minus .1ex}{-.8ex plus .2ex}
...
\section[MOSAIC Plugins]{\protect\llMOSAIC{} Plugins}
...
\section[MOSAIC Structured Reporting]{\protect\llMOSAIC{} Structured Reporting (\protect\lsMOSAICSR{})}
...
\section[MOSAIC Annotations]{\protect\llMOSAIC{} Annotations}
...
\section[MOSAIC in the context of Image Analysis]{\protect\llMOSAIC{} in the context of Image Analysis}
\p{This section will focus on one specific application 
of \MOSAICSR{} in the context of image analysis 
and bioimaging --- specifically, what LTS terms \DSPIN{} 
(\q{Data Structure 
...
DSPIN and MOSAICSR]{Meta-Procedural Modeling in 
...
\vspace{-.25em}
\section[D-SPIN in Contexts Supplemental to Image Processing]{\protect\lsDSPIN{} in Contexts Supplemental to Image Processing}
\vspace{.75em}
...
is a \sJava{} application with \sQt{} 
(i.e., \QtJambi{}) front-ends 
LTS is porting to \sCpp{}.  The supplemental 
...
\subsection[A Semantic DICOM Object Model]{A Semantic \protect\lsDICOM{} Object Model}
...
\subsection[Semantic DICOM via DCMTK Extensions]{Semantic \protect\lsDICOM{} via \protect\lsDCMTK{} Extensions}
...
\section[Markup Serialization and Grounding in 
MOSAIC Portals]{Markup Serialization and \protect\q{Grounding} in 
...
where ordinary \XML{} is expected and/or where 
\XML{} serves as a basis of standardization.  \lMOSAIC{} 
also provides code libraries 
...
{\newpage}
\section{Appendix: LTS's Integration between 
Different Flow Cytometry Libraries}
...
techniques discussed here could likewise be applied to other 
\FCM{} libraries).  In the \cytoLib{}/\FACSanadu{} reconciliation, 
...
would thus serve as an intermediary between \cytoLib{} 
and \FACSanadu{}, but they are also designed with an eye toward 
...
\begin{description}[leftmargin=2pt,itemsep=4pt]
...
different \textbf{coordinate} class when reading data via 
...
relative to the entire computer screen, or to one \GUI{} window, 
or to the top-left corner of the object whose method handles a 
given user-interaction event.  
A further complicating factor is that some coordinate 
systems are based on integer values, while others employ 
floating-point decimals.  Moreover, in contexts 
outside of (but related to) \FCM{}, such as  
\PDF{} rendering and image analysis, there 
are still further coordinate systems.  All told, 
a cross-disciplinary code base  
may need to manage \TwoD{} spatial positions 
recorded in at least seven different 
formats.  It is therefore a reasonable practice to factor 
...
(as standardized in \GatingML{}, for example), but other 
representations are used in other contexts, 
...
morphological quality from the ellipse axis 
angles, which are treated as a feature 
of how its intrinsic space 
...
for \cytoLib{}.  The \DSPIN{} solution 
prioritizes extensibility, with transformation classes 
structured to facilitate both subclassing \textit{and} 
\GUI{} integration.  This comports with a  
design philosophy for image-processing 
