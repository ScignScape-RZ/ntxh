...
hierarchical formats such as spreadsheets, 
...
Controled Vocabularies; so ontology constraints often 
...
so commonplace that a \q{data ontology} is unnecessary, 
...
so that researchers can describe symptoms via a canonically 
recognized term rather than through informal 
text descriptions.  In general, ontologies constrain 
data sets by stipulating that particular individual 
values within the overall data have a name or description 
...
applications which use them (along with those
