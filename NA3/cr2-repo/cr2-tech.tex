...
\titlespacing*{\section}
{0pt}{4.5ex plus 1ex minus .1ex}{2.2ex plus .2ex}
...
\ifnum\value{page}=23   %\pgfmathresult>0\relax
\put(12,56){
...
\geometry{left=.825in,top=.5in,right=.825in,bottom=1.2in} %margins
...
\p{LTS is building SARS-CoV-2/Covid-19 repositories 
to accelerate scientific discovery and therapeutic 
intervention.  To do so, LTS is developing a 
pair of data and code repositories 
...
techniques applicable both to data integration and to Artificial Intellegence 
...
projects such as \CBICA{} (Center for Biomedical Image 
Computing \& Analytics) and ReproZip (a tool for packaging 
...
and medical-outcome reporting, such as \OMOP{} 
...
Resources), \RadLex{} (Radiology Lexicon), and \LOINC{} 
(Logical Observation Identifiers Names and Codes).\footnote{%
Whereas these formats are normally used in the context of 
relational databases, the \CRtwo{} and \AIMConc{} programming 
tools are oriented toward custom data mining 
solutions as well as toward the implementation 
of specialized desktop-style software applications 
employing special-purpose \GUI{} classes.}
In general, \CRtwo{} and \AIMConc{} are structured 
in such a way that these repositories provide 
a development environment for custom application 
design, with an emphasis on \GUI{} 
implementations using the \Qt{} framework. 
%While these data formats are normally 
%used in the context of relational databases, the 
%\CRtwo{} and \AIMConc{} programming tools allows developers 
%to manipulate clinical data in formats natively recognized 
%by data-mining and \GUI{} code, which enables programmers 
%to apply sophisticated algorithms to clinical data and 
%also to present human users with convenient desktop-style 
%applications to initiate and view the results of these 
%analytic processes.  
Appendix 1 to this paper presents 
more information about the integration of 
clinical data with \GUI{} applications, with 
specific focus on integrating radiological and 
Comparative Effectiveness data.}

%  Appendix 1 to this paper presents 
%more information about Medical Outcomes code incorporated 
%into \CRtwo{} and \AIMConc{}.}

\p{In the specific field of radiological medical and diagnostic imaging, 
recent research has focused on how to effectively connect 
radiological data with medical outcomes and Comparative 
Effectiveness Research (\CER{}).  Traditionally, 
medical imaging formats such as \PACS{} (Picture Archiving 
and Communication System) and \DICOM{} (Digital Imaging 
and Communications in Medicine) have been largely separated 
from clinical records.  As a result, it has been difficult for 
clinical outcome researchers to cross-reference radiological data with 
clinical reports (clinical narratives, treatment protocols, etc.), 
indicating patient/subjects' subsequent course of treatment and 
corresponding outcomes.  Attempts have been 
made to remedy this information gap by the semantic 
annotation of radiological findings, leading to the development 
of imaging-related ontologies such as \RadLex{} and \SeDI{} 
(Semantic \DICOM{} Ontology).  One goal of 
\CRtwo{} is, accordingly, to generalize these semantic-imaging 
frameworks to the more flexible paradigms of 
\textit{hypergraph} ontologies --- schema defined 
on hypergraph instances, similar to how 
conventional ontologies define constraints on 
directed, labeled, \RDF{} (Resource Description Framework) 
style graphs.}

\p{Another domain which has gained 
significant public attention in the Covid-19 environment 
is vaccine engineering. 
In the field of vaccine development and 
infectious-disease research, conducting vaccine 
trials and vaccination campaigns 
generates a diverse spectrum of data profiles. 
This information ranges from clinical observational reports of 
the effectiveness of candidate vaccines during their development, to 
reports on the extent and effectiveness of immunization projects 
(which often depends on socioeconomic/geographic/political 
information in addition to the biomedical data).  Moreover, 
scientific understanding of the effectiveness of vaccines 
depends on assessments of the immunological response 
of vaccinated individuals (which may incoporate data from 
blood tests, molecular imaging, cytometry, serological profiling, 
and so forth) --- generating fundamentally different forms of data than 
encountered in the context of vaccine trials and 
vaccination compaigns.  Comprehensive support for vaccine-related 
data sets, therefore, requires software-development 
technologies that would be able to integrate 
and represent information from a wide range of 
laboratory and in-the-field methods.  As with radiology/outcomes 
integration, this heterogeneous data profile has 
led to the design of \RDF{} ontologies, notably the 
\q{VIOLIN} Vaccine Ontology.  Here again, \CRtwo{}, 
in order to accelerate research and documentation related to 
vaccine development, will innovatively expand 
the Vaccine Ontology to comport with hypergraph ontologies.  
More information on hypergraph ontologies, 
with specific reference to radiological and vaccine-related lexicons, 
is provided in the appendices.}

\section{Introduction}
\p{In an effort to accelerate scientific discovery and therapeutic 
intervention, LTS is engaged in the curation of 
the \CRtwo{} (\q{Cross-Disciplinary Repository 
for Covid-19 Research}) and \AIMConc{} 
(\q{\lAI{} Methodology and Conceptual Space Theory}) repositories.  
The purpose of such repositories is to 
...
technologies.  Part I of this paper will outline the \CRtwo{} 
and \AIMConc{} repositories; Part II will discuss the new technologies 
in greater detail.  These technologies have deep market 
...
on code that supports Artificial Intellegence and 
...
\part{Novel Database Engineering and Data-Set 
...
which are dicussed in the next section.}
