...
\geometry{left=.5in,top=.6in,right=.45in,bottom=1.35in} %margins
...
for the Scientific Community and Public Policymakers}
}}}
\end{center}
\end{minipage}
\end{tcolorbox}
...
to accurately capture first-hand experiential symptomatology of 
...
policymaking, integrating different levels of government as 
well as diverse modes of scientific inquiry.  Genomic, biomolecular, 
epidemiological, socio-demographic, clinical, and radiological information 
are all pertinent to Covid-19.  In this environment, it is 
important that the 
...
mandates across the entire country.  However it is also important 
that policy differences are a product of alternative interpretations of 
scientific knowledge, or of local communities having diverse needs 
...
corporate/academic contributions (exemplified by 
\Cnineteen{} and by institution-generated portals, respectively) 
to Covid-19 data sharing 
have been incomplete, for opposite but complementary 
reasons.  That is, \lCnineteen{} is highly structured and tightly 
integrated, but it focuses primarily on text mining and 
scientific documents, not \textit{research} data.  Specifically, 
...
\Cnineteen{} with the data-centric focus of several 
isolated projects sharing Covid-19 data 
with the scientific community, policy makers, 
and the general public.}

\section{Description}
\p{The design of \CRtwo{} derives from the principles outlined 
in the previous paragraph --- specifically, .
In particular, \CRtwo{} can provide value at different 
...
\CRtwo{} will be 
paired with a larger technology which shares a similar 
...
realized in clashing technologies (spreadsheets, 
...
amenable to graph-like representations.  For instance, 
...
\p{Using Applying \CRtwo{}'s  A higher level of data integration can then be 
...
biomedical context --- demands a careful balance between 
applications which are either too narrow or too broad 
...
of health workers with different specializations can be very 
different.  Conversely, doctors also complain about 
...
proper balance between these extremes is, one can say, 
...
be partitioned into two classes: one the 
one hand, domain-specific logic which reflects the 
quantitative or theoretical models of 
narrow scientific fields; on the other hand, 
...
\p{In short, this architecture conceives of a 
central hub which is responsible for storing 
data and for serving as a common access point 
--- providing the gateway where authorized 
users can gain access to heterogeneous information 
spaces utilized by an array of domain-specific 
software applications.  These peer applications  
would not be directly responsible for data persistence 
or user identity management, so they can focus 
...
managing multiple applications' information while 
...
\p{The multi-application networking architecture 
just outlined has precedents in current database and 
engineering technologies.  Many hospitals and 
...
\p{By contrast, the technology introduced with \CRtwo{} will 
develop multi-application networking in the context of more 
substantial, desktop-style scientific applications.  In sum, 
this technology offers a hybrid of the development methodologies 
employed for desktop scientific software and those 
applicable to multi-agent heterogeneous data stores, like 
a Semantic Data Lake.  In particular, \CRtwo{} will utilize 
...
\section{Augmenting Covid-19 Data with Patient Narratives}
...
descriptions, which may be useful both for diagnostic 
assessments and for gleaning information about 
Covid's psychological and neurological effects.}

\p{Here again \Cnineteen{} offers a useful case study.  
The \Cnineteen{} corpus has archived thousands of 
research papers in a common format suitable for text mining.  
This textual data was then made openly available to 
researchers and programmers in the hopes that developers 
would implement new text-mining algorithms to mine data 
or detect patterns in the overall corpus.  Just as 
\CRtwo{} envisions an analogous curation of research 
data sets for data mining, we can also see the 
benefit of a text-mining repository of patient narratives 
submitted by citizens who have been diagnosed with, 
or believe they may have, Covid-19.  \CRtwo{} will not 
specify how these narratives should be collected, but 
it will implement a common representational format 
so that patient narratives can be pooled, analogous 
to the merging of research manuscripts 
within \Cnineteen{}.}
...
patient narrative representations may be paired 
with \GUI{} or multi-media content.  For example, 
the software with which patients submit information 
could allow them to combine text-form narratives with 
graphics indicating the source of pain or 
discomfort; or narratives may be accompanied 
by audio files where patients cough or speak into 
a microphone.  A patient-narrative representation 
therefore must be flexible enough to include 
these possible forms of multi-media content.}

\p{As described earlier, an information space 
adapted for multiple peer applications should encompass 
capabilities for saving application state, which 
...
against anatomic silhouettes (back, front, head, and so forth).  
As patients use such a form, \GUI{} state corresponds 
to the subjective symptomology which should be 
incorporated into patient narratives and medical 
records.  This is an example of how 
application-persistence logic can be marshaled to 
the related project of curating patient narratives.}

\p{Once aggregated into research corpora, patient 
narratives are available to be analyzed by different 
language-processing and text-mining methodologies.  
Our strategies for encoding patient texts are based 
on Sequence Package Analysis, a novel discourse-representation 
technique developed by Amy Neustein (see 
\href{https://www.semanticscholar.org/paper/Using-Sequence-Package-Analysis-to-Improve-Natural-Neustein/422873abcd06164362350981bd7fbc7342746ac8}{Using Sequence Package Analysis to Improve Natural Language Understanding}).  
The patient-narrative models are also designed to 
facilitate annotation according to techniques described 
in Dr. Neustein's \href{https://www.semanticscholar.org/paper/1.-Application-of-text-mining-to-biomedical-and-Neustein-Imambi/bcf9d9fc1ddfc533f63de010c5f03e0d109659c6}{Application of Text Mining to Biomedical Knowledge Extraction: Analyzing Clinical Narratives
and Medical Literature}.}

\p{Further documentation of text-encoding methodology applicable 
both to patient narratives and to manuscripts associated 
