...
\put(137,126){
 {\setlength{\fboxsep}{.65em}
   ...
   {\color{pink!75!black}{\parbox{15.3cm}{\vspace{7pt}\framebox{\begin{minipage}{.5\textwidth}
	  {\textbf{\color{blGreen!40!blbl}{\Large For more information, please contact:} \\ 
	  {\color{blGreen!20!black}\hspace*{2em}Linguistic Technology Systems \\
      \hspace*{2em}amy.neustein@verizon.net
      \hspace*{.5em}\bullet{}\hspace*{.5em} \textbf{(917) 817-2184} }}}
	 \end{minipage}}}}}}
}
\fi
...
{\fontfamily{fvs}\fontsize{10}{11}\selectfont   
...
\section{Data Transparency and the LTS ``Dataset Creator"}
...
Research and Development process, as well as demand for a 
novel level of transparency and openness.  For example, 
...
Administration).  Data which \textit{is} publicly shared 
tends to be tied to research papers which are read by a 
relatively small, specialist audience.  All of this is 
...
policy makers, scientists, and journalists are 
looking for access to research data directly, 
rather than circuitously through academic publications.}

\p{\lCRtwo{} will introduce a new \q{Dataset Creator} technology 
targeted toward this new environment of direct, transparent 
data access.  Technically, Dataset Creator is a framework for 
constructing data sets which include computer code based on the 
\Qt{} application-development platform.  Data sets created 
...
can be difficult to reuse.  In particular, Research Objects 
should be (as much as possible) \textit{self-contained,} 
which means that scientists do not need external 
software to access and study the data --- any special 
...
is a very comprehensive framework encompassing 
...
files on user's computers (in this manner \Qt{} is 
different than most software packages, which usually 
demand a \q{system install}).}

\p{By leveraging the \Qt{} platform, Dataset Creator enables 
standalone, self-contained, and full-featured 
native applications to be uniquely implemented 
...
field but unfamiliar outside that field).  
Research Objects composed with Dataset Creator can 
also readily be integrated into Multi-Application Networks 
as described above, since the dataset applications 
are autonomous, native \GUI{} applications that can 
easily interoperate via \Qt{} messaging protocols.}

\p{For \CRtwo{}, the data sets included in the repository 
were created with a wide range of software products;  
many predate (or fail to apply) contemporary specifications 
such as the Research Object Protocol.  Therefore, not 
...
representation library.  \lCRtwo{} will also include 
\Qt{}-based software, such as a customized \PDF{} viewer, which 
will help researchers utilize the corpus in its 
entirety.  For example, the \CRtwo{} \PDF{} application 
will include special code to connet \PDF{} files 
with data sets via \q{micro-citations,} discussed in the 
next section.}


\section{Supporting Data Micro-Citations to Improve Machine Readability 
(Vaccine Trials and Ultraviolet Ray Data Model Examples)}

\p{The \CRtwo{} database engine supports annotating individual components 
of a database, technology sometimes referred to as 
\q{micro-citation}.  Data microcitations are references to 
integral parts of a data set, such as an individual table, 
or a single row/record or column in a table.  Microcitations allow 
these integral parts within the data set to be cited by and linked 
to publications, for purposes of machine readility and 
attribution.  For example, preliminary vaccine trials often 
target a patient cohort selected for demographic or medical 
criteria matching the population who would most benefit from 
the vaccine.  These criteria may be described in the texts of 
articles, but they are also identified by socio-demographic data 
which is part of the information generated by the trial: 
each patient has both a demographic profile and a 
innoculation outcome.  By making these connections explicit, 
text and data mining can be \textit{merged} as analytic 
tools targeting a data repository.  
%\lCRtwo{} is therefore 
%designed to optimize the convenience and rigor of micro-citations 
%both within data sets and within research papers.  
Software is then able to mine not just article text but the data 
on (for example) patient demographics and medical outcomes.}
...
controled vocabularies (such as a list of drug names, 
diseases, proteins, etc.).  
For instance, microcitations allow table columns to be 
...
(2) to enable granular links to be established between 
publications and data sets, making it easier for researchers 
to find the specific information most 
relevant to their own research 
(by joining specific parts of articles to corresponding
parts of the data set, analogous to hyperlinks 
between web pages).}
