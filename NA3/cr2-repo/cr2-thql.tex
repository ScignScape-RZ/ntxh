...
\put(59,953){\textbf{{\fontfamily{phv}\fontsize{14}{14}\selectfont{}EXECUTIVE SUMMARY}}}
\fi
}	
...
\put(16,56){
 {\setlength{\fboxsep}{.65em}
   ...
   {\color{white}{\parbox{10cm}{\vspace{7pt}\framebox{\begin{minipage}{.42\textwidth}
...
\geometry{left=.5in,top=.45in,right=.45in,bottom=1.2in} %margins
...
{\setlength{\fboxsep}{32pt}
	\relscale{1.2}{{\fontfamily{qcr}\fontseries{b}\selectfont%
{New LTS Tools to Accelerate Covid-19 Research}
}}}
\end{center}
\end{minipage}
\end{tcolorbox}
...
\p{The Cross-Disciplinary Repository for Covid-19 Research 
...
of text and data mining.  This repository is 
an opportunity for LTS to develop and showcase 
new database engineering and data-set construction 
technologies.  Both the new technologies and the 
\CRtwo{} repository will be outlined in the following 
sections.}
...
The second repository includes \q{preview} code sampling 
database engine features (the logic for constructing a 
database in shared memory, encoding data types for persistence, 
and so forth).  Companies or research groups interested in a more 
substantial code preview are invited to contact LTS, where 
we can discuss how the data management tools developed 
for \CRtwo{} can be customized for your own projects.}

\section{Overview of the New Technology}
\p{The main challenge when curating a 
data repository such as \CRtwo{} is reconciling 
heterogeneous data formats.  
In response to these problems, LTS has 
...
a Hypergraph Exchange Format (\HGXF{}) 
...
etc.).  In addition, \CRtwo{} will 
introduce a new protocol for engineering 
hypergraph databases, dubbed \THQL{} 
(Transparent Hypergraph Query Language).  
Databases conformant to the \THQL{} model will 
be able to export data in hypergraph-based 
formats, thereby generating a data set 
which can be used as a published, citable 
Research Object.  \lCRtwo{} will also introduce a 
new database engine (called DigammaDB) which 
serves as a \q{Reference Implementation} for 
\THQL{}.  In \CRtwo{}, DigammaDB is used 
to curate data sets before their final 
form is exported into the main data repository.  
However, DigammaDB is only a prototype and 
reference demonstration for \THQL{}; 
Customized commercial versions of a \THQL{} 
engine can be tailored to each project's 
data storage and 
...
easy to implement \GUI{} components which 
are initialized with data obtained from a 
\THQL{} database.  In short, \THQL{} 
...
The pre-eminent feature of Dataset Creator is 
...
Dataset Application will provide \q{data-access} 
code libraries for 
parsing the raw data-set files, so as to obtain 
the information visualized within the 
Dataset Application's \GUI{} classes.  
These data-access libraries are also useful for 
providing machine-readable access to 
such raw data.  Subsequent researchers can then 
re-use the data-access code so as to transform, 
...
to publications, for purposes of machine readility and 
...
data which is part of the information \makebox{generated 
by the trial. \hspace{-6pt} By making these connections between criteria 
discussed in the article and those 
rep-} resented in the corresponding data set explicit, 
...
a microphone.  In light of this 
range of possible inputs, a patient-narrative 
encoding must, therefore, be flexible enough to 
...
in this way the graphics-based represention of symptoms 
...
\section{Native Application Development with THQL}
...
integrate seamlessly into the client application  
...
be instantiated either by a special language 
with its own syntax and semantics 
(analogous the \SQL{} or \SPARQL{}), or 
...
achieved by exposing each role-procedure 
to a scripting interface.  For example, 
\THQL{}'s Reference Implementation, 
DigammaDB, exposes each protocol-specific 
procedure via a set of globally 
accessible pointers to \Cpp{} functions.  
Parsing a query languags is then a 
straightforward process of mapping query 
expressions to the requisite procedure, 
whose corresponding handle can then be 
obtained via \Cpp{} interop.}

\p{As indicated by its name, \THQL{} 
...
(such as hypernodes, frames, and subgraphs) which 
...
arbitratily deep nested structures.}

\p{In addition to the operations just outlined, 
\THQL{} recognizes additional stuctures 
corresponding to conceptual details described 
above --- for example, fields within 
hypernodes can be linked with 
dimensional attributes (e.g. scales and 
units of measurement) and identified 
as microcitation targets.  \lTHQL{} likewise 
supports controled vocabularies, 
called \q{dominions} (which stands for 
Domain-Specific Mini-Ontologies), applying to 
hypernode-types and/or connection labels.  A graph 
can then be configured to only accept hypernodes 
which conform to one of the dominion-defined 
types; or to only allow connections to be 
...
cofiguring each database to support different 
...
persisted data storage is implemented via the WhiteDB 
...
accessed from multiple applications).  DigammaDB encodes 
hypernodes in WhiteDB records (although programmers 
can interface with the underlying WhiteDB instances if 
desired).  DigammaDB can then use WhiteDB's index and 
query mechanism as the foundation for its own 
higher-level query system.  To support different 
\THQL{} operational modes, DigammaDB organizes a 
stage-structure based on encoding WhiteDB values: 
...
(including sent over a network) by storing all 
database information in a special file format.  
DigammaDB instances can be shared via this 
same mechanism, although another option is to 
export the contents of a DigammaDB database to 
...
In this guise DigammaDB works in conjunction 
with \dsC{}, serving as the engine to 
construct a data set publishing research data 
curated via a DigammaDB database.  Our 
...
Dataset Applications in a \Qt{} context; \dsC{} 
projects are structured to use \Qt{}'s \q{qmake} 
...
Data set users are also classified into several 
...
the data sets themselves), \dsC{} recognizes 
...
\LaTeX{}-to-\PDF{} convertors, generate \HGXF{} 
files which store information about textual and 
\PDF{} viewport coordinates locating semantically 
meaningful elements, such as sentences and annotations.  
These files are then zipped and embedded in 
\PDF{} files.  With \dsC{}, these \PDF{} files can 
then be loaded into a customized \PDF{} viewer which 
can read the embedded \HGXF{} data.  This allows the 
...
the Dataset Applications \GUI{} implementation, 
and vice-versa.  For example, a researchers reading 
...
where microcitations are practically useful: 
...
links that the Dataset Application can use to launch 
a \PDF{} window opened to the page and location where 
the corresponding conception is discussed in the article.}
...
web site, such as \href{https://raw.githubusercontent.com/Mosaic-DigammaDB/CRCR/master/cr2.pdf}{here} (this is a downloadable \PDF{} link; 
visit the repository to see the larger archive structure).  
The document just referenced contains more information 
on \dsC{}, \HGXF{}, \HTXN{}, and other technologies 
discussed here.}
