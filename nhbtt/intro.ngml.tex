...
\p{To cite a concrete example, Ant\'onio Teixeira (\textit{et. al.}, this 
volume) consider \q{smart appliance} refrigerators which are aware of 
the door being left open, and even may track the expiration date of 
items inside.  Teixeira mentions the possibility that this data be 
viewed on a smart phone.  On the other hand, consider an industrial 
...
items (maybe with their expiration dates) can be 
presented as a \GUI{} component, perhaps occupying a 
separate application window organized as a list 
\q{widget}, with one food item per line 
...
so forth, given a numeric item identifier.  One question 
then is where this database would be hosted 
\mdash{} on a computer in the refrigerator owner's apartment, 
or a remote server, etc. \mdash{} and how it would be 
...
\p{This points to another trend: insofar as users are seen as 
seeking app-based (rather than desktop) access to 
CyberPhysical data, engineers appear to feel pressure to 
...
\p{Time will tell how well \AI{} will enhance 
the CyberPhysical domain, but I think there is 
genuine risk that industry attention could become 
so focused on \AI{} and \q{big data} 
mining and acquisition that other software 
engineering and Human-Computer Interaction 
challenges will be given short shrift.  
This could potentially hinder rather than 
help the evolution of CyberPhysical technology overall.  
But notice how this reliance on \AI{} is driven 
not only by a desire for \q{smart} analytics or 
increased productivity (as in \AI{}'s industrial 
role in robotics, image processing, and so on); 
instead, \AI{} (plus speech-recognition for 
voice-oriented \HCI{}) comes to the fore in 
order to compensate for the limitations of 
User Interfaces in small-screen, touch-screen, 
or even no-screen (e.g., voice) contexts.      
}
\p{In short, transitioning from desktop-oriented 
architectural models to more light-weight 
smart-phone front ends (and \AI{}-powered 
back ends) does not necessarily 
bring us forward from a safety, software 
quality, or User Experience point of view.  
On these considerations I propose here that 
\q{hub} software models should still be seen 
as establishing the core data representations 
and operational protocols through which 
CyberPhysical technologies and requirements 
can be standardized, from which alternative 
client designs (like phone apps) can build 
off.  I will return to this theme in 
case-studies treated in Section 1.  
}
\p{As I argued, in any case, software hubs have different design 
...
remains hidden (this chapter's demo code exercises one
strategy for enforcing that distinction).  These seem like common-sense requirements.
