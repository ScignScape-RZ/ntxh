...
and also employment of data-publishing standards 
...
\p{Alongside these demo sets, an additional demonstrative
example is provided addressing biomedical device profiles 
and patient data (including privacy and security concerns).  
The focus of these latter examples is more architectural: 
\Cpp{} classes are organized to partition patient-specific 
data points into multiple layers (such as private data,
demographic data, and \q{cohort models} in the sense 
of \cite{EranBellin} which permit detailed course-of-treatment 
and treatment outcomes to be represented without sacrificing 
privacy), along with encryption techniques for the most 
sensitive infomation layers.  The implementation 
demonstrates methods for sharing medically significant 
data with some degree of detail while also constraining 
access to secure layers within that data as necessary, 
illustrating how \GUI{}-based applications can both 
adapt to the lack of securitized data as well as 
proper protocols for accessing securitized layers.  The demo 
also prototypes integration of patient-centric layers 
with biomedical device profiles, so that the abovementioned 
patient information models can be leveraged in the 
context of software managing CyberPhysical health data 
(including \q{Ubiquitous Sensing for Healthcare}-style 
input streams). 
}
\p{Philosophically, a \q{Procedural} Ontology supplements
the \q{static} semantic model of data structures, which
expresses \i{internal} guarantees on information content
and representation.  Structural and representational guarantees
usually come into play in the event of data being shared
between two distinct computing environments; as such,
the full interaction space partners data structures themselves
with \i{senders} and \i{receivers}.  Procedural Ontology,
then, expresses requirements on senders and receivers analogous
to conventional Ontologies' semantics on data structures
internally.
}
\p{The \i{internal} or \i{static}, and
\i{external} or \i{procedural}, dimensions of a
Procedural Ontogy, respectively, may be conceptually 
unified via a common type-theoretic foundation.
Notionally, \i{types} and \i{procedures} are
interdependent: there are no types without
procedures to construct, modify, or utilize their
values; likewise there are no procedures without
types to circumscribe their parameters.
Theoretically, then, \i{static} and \i{procedural}
Ontologies are similarly interdependent:
internal semantic models are only operationally
meaningful in the context of data re-use, which
implies information being replicated, and so
an origin and destination point.
Interface definition for software clients and
servers is of course a familiar aspect of
software engineering; the new concepts I am
proposing here involve nonstandard,
hypergraph-based representations of
procedure types and requirements, which translates
to Hypergraph Ontologies structurally
different (and, I would argue, more expressive)
than conventional Ontologies or Interface
Definition Languages.
}
\p{As a practical matter, the demo set illustrates techniques
for creating and leveraging a Procedural Ontology
in the context of hypothetical biomedical patient and
device data.  The \q{Ontologies} (technically
expressed in a hypergraph structure rather than
Semantic Web format) define conditions on
senders and receivers,
which are requisite for software components
to participate in data sharing \mdash{} and therefore
also guarantees on components' behavior, at least
up to the level of detail offered by the Ontology.
The larger point is that static Ontologies are only
one part of the distributed but
regulated communications ecosystem
envisioned by projects like the
Semantic Web: full realization of
this technology's potential depends
on procedure- and software-level
Requirements Engineering \mdash{} which suggests that new
...
I hope to show by reworking the demo's data sets how
...
\cite{BhavaniThuraisingham}).  The dangers
