...
RZ_Phaon_Block::RZ_Phaon_Block(caon_ptr<RZ_Phaon_Block> parent_block)
...
int RZ_Phaon_Block::get_new_hd_code()
{
 static int result = 0;
 return ++result;
}
  ...
  RE_Node& entry_node, QString pgbs, caon_ptr<RE_Block_Entry> rbe)
{
 if(caon_ptr<RE_Node> start_node = visitor_phaon.start_node_from_call_entry_node(&entry_node))
 {
  ...
  pgb(header_step_forms_).copy_value("&fsym-node", "&channel-seq");
  while(current_node)
  {
...
void RZ_Phaon_Block::add_form_from_call_entry_node(RZ_Graph_Visitor_Phaon& visitor_phaon,
  RE_Node& entry_node)
{
 if(current_form_)
 {
...
void RZ_Phaon_Block::scan_form_from_start_node(RZ_Graph_Visitor_Phaon& visitor_phaon,
  RE_Node& start_node)
{
...
caon_ptr<RE_Block_Entry> RZ_Phaon_Block::get_block_entry()
{
 if(block_info_)
 {
...
void RZ_Phaon_Block::scan_form_from_statement_entry_node(RZ_Graph_Visitor_Phaon& visitor_phaon,
  RE_Node& start_node)
{
 int lambda_count = 0;
  ...
  case RZ_Lisp_Graph_Visitor::Next_Node_Premise::Block_Entry:
   {
    ...
    caon_ptr<RZ_Phaon_Block> new_block = new RZ_Phaon_Block(this);
    caon_ptr<RZ_Phaon_Block> current_block = new_block;
    ...
    if(caon_ptr<RE_Node> cen = visitor_phaon.call_entry_node_from_block_entry_node(next_node))
    {
     ...
     while(continue_node)
     {
      ...
      caon_ptr<RZ_Phaon_Block> nb = new RZ_Phaon_Block(this);
