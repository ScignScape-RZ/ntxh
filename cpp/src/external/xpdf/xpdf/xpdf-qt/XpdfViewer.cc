...
XpdfViewer *XpdfViewer::create(Xpdf_Component *xpc, QString fileName, int page,
          QString destName, QString password,
          GBool fullScreen) {
  XpdfViewer *viewer;

  viewer = new XpdfViewer(xpc, fullScreen);
  if (!viewer->open(fileName, page, destName, password)) {
    viewer->close();
    delete viewer;
    return NULL;
...
void XpdfViewer::createWindow() {
  errorWindow = new XpdfErrorWindow(this,
    app? app->getErrorEventType() : xpdfc_->getErrorEventType());
