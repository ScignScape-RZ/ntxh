...
If you use "`git clone`", it is recommended to provide your own name to the folder where the data set is unpacked.  For example, create a folder called "`ntxh-DependencyGrammar`" or something smaller ("`ntxh`", say) and _inside_ that folder execute "`git clone https://github.com/scignscape-rz/ntxh.git ar`" -- notice the trailing "`ar`" (for "archive") where this repository will be unpacked.  Then the parent "`ntxh`" folder can be used for other files related to the project (or follow-up research) but isolated from the actual repo.
